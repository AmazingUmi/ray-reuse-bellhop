MODULE bdry3Dmod
  ...
  TYPE HSInfo2
     ...
     REAL     (KIND=8) :: vr, alpha2_f                  ! grain size info (sound speed ratio, density, attenuation)
...
CONTAINS
  ...
  SUBROUTINE ReadBTY3D( FileRoot, BotBTY, DepthB, BC, PRTFile )
    ...
    CHARACTER ( LEN=80 ), INTENT( IN ) :: FileRoot
    INTEGER :: iProv
    REAL ( KIND=8 ) :: Mz, vr, rhor, alpha2_f    ! grain size info
    REAL ( KIND=8 ), ALLOCATABLE :: Temp( : )
    ...
    CASE ( '~', '*' )
       ...
       IF ( btyType( 2 : 2 ) == 'L' ) THEN  ! Long format
          ...
          CASE( 'G' )
             ! read in geoacoustic properties for each province
             WRITE( PRTFile, * )
             WRITE( PRTFile, * ) 'Province grain-size properties'
             READ(  BTYFile, * ) NBotProvinces
             WRITE( PRTFILE, * ) 'Number of bottom provinces = ', NBotProvinces
             ...
             WRITE( PRTFile, "( /, 'Province     Mz'        )" )
             WRITE( PRTFile, "(    '        (phi units)', / )" )
             DO iProv = 1, NBotProvinces
                READ(  BTYFile, * ) Mz
                WRITE( PRTFile, FMT="( I5, 2X, 2F10.2 )" ) iProv, Mz

                CALL GrainSize_to_Geoacoustic( Mz, vr, rhor, alpha2_f )

                BotProv( iProv )%vr       = vr
                BotProv( iProv )%rho      = rhor
                BotProv( iProv )%alpha2_f = alpha2_f
             END DO
          ...
          CASE( 'F' )
             CALL ERROUT( 'BELLHOP3D: ReadBTY3D', 'Tabulated reflection coefficient not currently implemented' )
          END SELECT BCselect
       END IF
  ...
  SUBROUTINE GetTopSeg3D( x )
    ...
    IF ( x( 1 ) < xTopSeg( 1 ) .OR. x( 1 ) > xTopSeg( 2 ) ) THEN
       ...
       ELSE
          ...
          CALL ERROUT( 'BELLHOP3D: GetTopSeg3D', 'Ray entered a segment where Altimetry is undefined' )
      END IF
    END IF
...
!!$       END IF
       ...
       ELSE
          ...
          ! CALL ERROUT( 'BELLHOP3D: GetBotSeg3D', 'Ray entered a segment where Bathymetry is undefined' )
       END IF
