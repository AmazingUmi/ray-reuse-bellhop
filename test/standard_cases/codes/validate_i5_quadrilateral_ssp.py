...
def generation_commands(
    ...
    if broadband_executable is not None:
        versions = versions + (("broadband",),)
        executable_for["broadband"] = "Bellhop_Broadband/build/release/bellhop_broadband"
...
def validate(
    ...
    if len(set(executables.values())) != len(executables) or len(
        set(executable_hashes.values())
    ) != len(executable_hashes):
        raise ValueError(
    ...
    # frequency slices, so comparisons below always read the candidate slice
    # at the frequency index instead of slice 0. Provenance for Broadband rests
    # on the same manifest identity/freshness and canonical sidecar checks
    # performed by load_profile; per-frequency rendered-env aggregates are not
    # comparable because the broadband invocation stages a single environment.
    loaded_broadband: dict[str, dict[str, list[tuple[Path, Path, Path]]]] = {}
    if "broadband" in executables:
        for case_id in CASES:
            loaded_broadband[case_id] = {}
            for profile, frequencies in PROFILES.items():
                loaded_broadband[case_id][profile] = load_profile(
                    results_root,
                    "broadband",
                    case_id,
                    profile,
    ...
    for profile, frequencies in PROFILES.items():
        for index, frequency in enumerate(frequencies):
            for case_id in CASES:
                ...
                if loaded_broadband:
                    broadband_shade = loaded_broadband[case_id][profile][
                        index
                    ][1]
                    passed, metrics = compare_files(
                        origin_shade,
                    ...
                    if not passed:
                        raise ValueError(
                            f"broadband/{case_id}/{profile}/"
                            f"{frequency:g}Hz field mismatch: {metrics}"
                        )
                    broadband_comparisons[
            ...
            if loaded_broadband:
                dependent_pressure = ShdReader(
                    loaded_broadband[RANGE_DEPENDENT_CASE][profile][index][1]
                ).read(frequency_index=index).pressure
                control_pressure = ShdReader(
                    loaded_broadband[CONTROL_CASE][profile][index][1]
                ).read(frequency_index=index).pressure
                if dependent_pressure.shape != control_pressure.shape:
                    raise ValueError(
                        f"broadband/{profile}/{frequency:g}Hz control shape "
                        "mismatch"
                    )
                maximum_difference = float(
                    np.max(np.abs(dependent_pressure - control_pressure))
                ...
                if (
                    ...
                    raise ValueError(
                        f"broadband/{profile}/{frequency:g}Hz "
                        "range-dependent SSP appears inactive "
                        f"({maximum_difference})"
                    )
                effect_guards[f"broadband/{profile}/{frequency:g}Hz"] = {
                    "passed": True,
                    "max_pressure_absolute_vs_q_control": maximum_difference,
                }
    ...
    if loaded_broadband:
        broadband_field_paths = [
            loaded_broadband[case_id][profile][index][1]
            for case_id in CASES
            for profile, frequencies in PROFILES.items()
            for index in range(len(frequencies))
        ]
        result["origin_broadband_field_comparisons"] = broadband_comparisons
        result["sha256"]["broadband_executable"] = executable_hashes[
            "broadband"
        ]
        result["sha256"]["broadband_field_aggregate"] = aggregate_sha256(
            broadband_field_paths
        )
        result["generation"]["broadband_case_commands"] = (
            generation_commands(executables["broadband"])
        )
        result["generation"]["broadband_validator_command"] = (
            "python3 "
            "test/standard_cases/codes/validate_i5_quadrilateral_ssp.py "
            "--results-root <results-root> "
            "--origin-executable Bellhop_origin/bin/bellhop "
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "optional Bellhop Broadband executable; when given, Broadband "
            "results in the results root are compared against Origin with "
            "the same tolerances"
        ),
    )
