...
CONTAINS
  SUBROUTINE ReadEnvironment( FileRoot, ThreeD )
    ...
    IF ( Bdry%Top%HS%BC == 'A' ) THEN
       ...
       WRITE( PRTFile, "(     '     (m)         (m/s)      (m/s)   (g/cm^3)                     ', / )" )
    END IF
    ...
    IF ( ThreeD ) THEN
       ...
       WRITE( PRTFile, * )
       WRITE( PRTFile, fmt = '(  '' Maximum ray range (m): '' )' )
       WRITE( PRTFile, fmt = '(  ''   Box%x = '', G11.4 )' ) Beam%Box%x
       WRITE( PRTFile, fmt = '(  ''   Box%y = '', G11.4 )' ) Beam%Box%y
       WRITE( PRTFile, fmt = '(  ''   Box%z = '', G11.4 )' ) Beam%Box%z
    ELSE
       ...
       WRITE( PRTFile, fmt = '(  '' Step length,       deltas = '', G11.4, '' m'' )' ) Beam%deltas
       WRITE( PRTFile, * )
       WRITE( PRTFile, fmt = '(  '' Maximum ray depth-range: '' )' )
       WRITE( PRTFile, fmt = '(  ''   Box%z  = '', G11.4, '' m'' )' ) Beam%Box%z
       WRITE( PRTFile, fmt = '(  ''   Box%r  = '', G11.4, ''km'' )' ) Beam%Box%r
    ...
    IF ( Beam%RunType( 1 : 1 ) /= 'R' ) THEN   ! no worry about the beam type if this is a ray trace run
       ...
       CASE ( 'R', 'C' )                        ! Cerveny Gaussian Beams; read extra lines to specify the beam options
          ...
          CASE DEFAULT
             CALL ERROUT( 'ReadEnvironment', 'Unknown curvature condition' )
          END SELECT
       ...
       CASE DEFAULT
          CALL ERROUT( 'ReadEnvironment', 'Unknown beam type (second letter of run type)' )
       END SELECT
    END IF
  ...
  SUBROUTINE ReadTopOpt( TopOpt, BC, AttenUnit, FileRoot )
    ...
    CASE ( 'Q' )
       ...
       IF ( IOSTAT /= 0 ) THEN   ! successful open?
          ...
          CALL ERROUT( 'BELLHOP - ReadTopOpt', 'Unable to open the SSP file' )
       END IF
    CASE ( 'H' )
       ...
       IF ( IOSTAT /= 0 ) THEN   ! successful open?
          WRITE( PRTFile, * ) 'SSPFile = ', TRIM( FileRoot ) // '.ssp'
          CALL ERROUT( 'BELLHOP - ReatTopOpt', 'Unable to open the SSP file' )
       END IF
    CASE ( 'A' )
       WRITE( PRTFile, * ) '    Analytic SSP option'
    CASE DEFAULT
       CALL ERROUT( 'ReadTopOpt', 'Unknown option for SSP approximation' )
    END SELECT
    ...
    CASE DEFAULT
       CALL ERROUT( 'ReadTopOpt', 'Unknown attenuation units' )
    END SELECT
    ...
    CASE ( ' ' )
    CASE DEFAULT
       CALL ERROUT( 'ReadTopOpt', 'Unknown top option letter in fifth position' )
    END SELECT
    ...
    CASE ( ' ' )
    CASE DEFAULT
       CALL ERROUT( 'ReadTopOpt', 'Unknown top option letter in sixth position' )
    END SELECT
  ...
  SUBROUTINE ReadRunType( RunType, PlotType )
    ...
    CASE DEFAULT
       CALL ERROUT( 'ReadRunType', 'Unknown RunType selected' )
    END SELECT
    ...
    CASE ( 'I' )
       ...
       IF ( Pos%NRz /= Pos%NRr ) CALL ERROUT( 'ReadRunType', 'Irregular grid option selected with NRz not equal to Nr' )
       PlotType = 'irregular '
    CASE DEFAULT
  ...
  SUBROUTINE TopBot( freq, AttenUnit, HS )
    ...
    CASE ( 'G' )                  ! *** Grain size (formulas from UW-APL HF Handbook)
       ...
       WRITE( PRTFile, FMT = "( 'Grain size =', F10.2 )" ) Mz
