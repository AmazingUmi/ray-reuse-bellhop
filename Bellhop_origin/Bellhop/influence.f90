...
!!! following assumes uniform spacing in Pos%r
          irA = MAX( MIN( INT( ( rA - Pos%Rr( 1 ) ) / Pos%Delta_r ) + 1, Pos%NRr ), 0 ) ! index of receiver
       END IF
       ...
       Stepping: DO iS = 2, Beam%Nsteps
          ...
          !!! following assumes uniform spacing in Pos%r
          irB = MAX( MIN( INT( ( rB - Pos%Rr( 1 ) ) / Pos%Delta_r ) + 1, Pos%NRr ), 0 ) ! index of receiver
  ...
  SUBROUTINE InfluenceGeoGaussianCart( U, alpha, Dalpha )
    ...
    COMPLEX,       INTENT( INOUT ) :: U( NRz_per_range, Pos%NRr )  ! complex pressure field
    INTEGER          :: irT( 1 ), irTT
    REAL    (KIND=8) :: x_ray( 2 ), rayt( 2 ), rayn( 2 ), x_rcvr( 2 ), rLen, RadiusMax, zMin, zMax, sigma, lambda, A, dqds, sigma1
    COMPLEX (KIND=8) :: dtauds
       ...
       RcvrRanges: DO
          ...
          IF ( Pos%Rr( ir ) >= MIN( rA, rB ) .AND. Pos%Rr( ir ) < MAX( rA, rB ) ) THEN
             ...
             RcvrDepths: DO iz = 1, NRz_per_range
                ...
                sigma1  = MAX( sigma, MIN( 0.2 * freq * REAL( ray2D( iS )%tau ), pi * lambda ) )  ! min pi * lambda, unless near
                !!!write( *, * ) is, sigma, sigma1
                !!!!!!sigma  = MAX( sigma, MIN( 0.2 * freq * REAL( ray2D( iS )%tau ), pi * lambda ) )  ! min pi * lambda, unless near
                !!!!!!!!!!!sigma  = MAX( sigma, pi * lambda )  ! min pi * lambda, unless near
                
                IF ( n < BeamWindow * sigma1 ) THEN   ! Within beam window?
                   delay    = ray2D( iS - 1 )%tau + s * dtauds     ! interpolated delay

                   !const    = Ratio1 * SQRT( ray2D( iS )%c / ( q0 * sigma ) ) * ray2D( iS )%Amp
                   !W        = ( sigma / sigma1 ) * EXP( -0.5 * ( n / sigma1 ) ** 2 )   ! Gaussian decay

                   const    = Ratio1 * SQRT( ray2D( iS )%c / ( q0 * sigma1 ) ) * ray2D( iS )%Amp
                   W        = SQRT( sigma / sigma1 ) * EXP( -0.5 * ( n / sigma1 ) ** 2 )   ! Gaussian decay
                   !!!A        = ABS( q0 / q )
                   !!!const    = Ratio1 * SQRT( ray2D( iS )%c / ABS( q ) ) * ray2D( iS )%Amp
