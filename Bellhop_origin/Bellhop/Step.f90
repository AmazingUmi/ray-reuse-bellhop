...
CONTAINS
  ...
  SUBROUTINE Step2D( ray0, ray2, Topx, Topn, Botx, Botn, StepInfo )
    ...
    IF ( iSegz /= iSegz0 .OR. iSegr /= iSegr0 ) THEN
       ...
       CASE ( 'N', 'C', 'Q', 'H' )  ! N2-linear or C-linear profile option
          ...
          !!! what if we cross isegr or isegz at the same time?
  ...
  SUBROUTINE ReduceStep2D( x0, urayt, iSegz0, iSegr0, Topx, Topn, Botx, Botn, h )
    ...
    IF ( h < 1.0d-3 * Beam%deltas ) THEN   ! is it taking an infinitesimal step?
       h = 1.0d-3 * Beam%deltas            ! make sure we make some motion
       iSmallStepCtr = iSmallStepCtr + 1   ! keep a count of the number of sequential small steps
       ! IF ( iSmallStepCtr > 10 ) h = Beam%deltas   ! if stuck, take the default step
       ! write( *, * ) 'h used', h, iSmallStepCtr
    ELSE
