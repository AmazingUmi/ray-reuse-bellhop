...
[[nodiscard]] std::size_t checkedProduct(std::size_t lhs, std::size_t rhs) {
  if (lhs != 0U && rhs > std::numeric_limits<std::size_t>::max() / lhs) {
    throw ValidationError(
        "fused pressure-workspace dimensions overflow size_t");
  }
  return lhs * rhs;
}
...
FusedPressureWorkspace::FusedPressureWorkspace(const ReceiverGrid& receivers,
...
std::size_t FusedPressureWorkspace::cellOffset(std::size_t rangeIndex,
