...
output="${BROADBAND_INTERMEDIATE_OUTPUT:-${project_root}/test/standard_cases/results/intermediate_state_matrix.json}"
...
"${python_command[@]}" \
  ...
  --broadband-probe "${project_root}/Bellhop_Broadband/build/release/bellhop_broadband_geometry_oracle_probe" \
  --output "${output}"
