...
repetitions="${BROADBAND_MICROBENCH_REPETITIONS:-3}"
warmups="${BROADBAND_MICROBENCH_WARMUPS:-1}"
output="${BROADBAND_MICROBENCH_OUTPUT:-${project_root}/test/standard_cases/results/single_thread_microbenchmark.json}"
...
"${python_command[@]}" \
  ...
  --broadband-executable "${project_root}/Bellhop_Broadband/build/release/bellhop_broadband" \
