...
if grep -R -nE \
    ...
    "${forbidden_pattern}" \
    "${project_root}"; then
  echo "Broadband source or CMake contains an F2CPP dependency" >&2
  exit 1
fi
...
for build_directory in \
  ...
  if grep -R -nE \
    ...
    echo "Broadband build metadata contains an F2CPP dependency" >&2
    exit 1
  fi
done
...
if [[ -x "${release_executable}" ]]; then
  if command -v otool >/dev/null 2>&1; then
    if otool -L "${release_executable}" |
      ...
      echo "Broadband executable links an F2CPP library" >&2
      exit 1
    fi
...
echo "Bellhop_Broadband independence checks passed"
