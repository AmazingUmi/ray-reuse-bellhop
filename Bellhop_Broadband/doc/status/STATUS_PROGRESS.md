...
> 改动本轮保持未提交）
> 当前状态：`Bellhop_F2CPP → Bellhop_Broadband` Production Feature Parity 序列
...
BB-M1 将产品从 Bellhop Broadband 演进为 **Bellhop Broadband**：executable
`bellhop_broadband`、CMake project `BellhopBroadband`、package
`bellhop-broadband`（`Bellhop_Broadband/` 目录名与内部 namespace/include 根
...
**Bellhop_F2CPP → Bellhop_Broadband Production Feature Parity COMPLETE**。
