...
const char* terminationName(broadband::RayTerminationReason reason) {
  switch (reason) {
    case broadband::RayTerminationReason::ExitedDomain:
      return "ExitedDomain";
    case broadband::RayTerminationReason::NumericalFailure:
      return "NumericalFailure";
    case broadband::RayTerminationReason::PointLimit:
      return "PointLimit";
  }
  return "Unknown";
}
...
broadband::Environment makeI5QuadrilateralEnvironment() {
  const auto grid = std::make_shared<const broadband::QuadrilateralSspGrid>(
      broadband::QuadrilateralSspGrid{
          .rangesMeters = {0.0, 350.0, 800.0},
          .speedsDepthMajor = {1500.0, 1540.0, 1580.0, 1500.0, 1520.0, 1540.0},
          .depthCount = 2U,
          .rangeCount = 3U});
  return broadband::Environment(
      broadband::SoundSpeedProfile(
          {{.depth = 0.0, .soundSpeed = 1500.0, .density = 1000.0},
           {.depth = 100.0, .soundSpeed = 1500.0, .density = 1000.0}},
          broadband::SspInterpolationKind::Quadrilateral, grid),
      broadband::BoundaryModel::vacuum(0.0),
      broadband::BoundaryModel::rigid(100.0));
}
...
int main(int argc, char* argv[]) {
  ...
  if (argc == 4 && !useMunkConfiguration && !useCurvilinear &&
      !useI5Quadrilateral) {
  ...
  const broadband::Environment environment =
      useMunkConfiguration
                              ...
                              broadband::SoundSpeedProfile(
                                  {{.depth = 0.0,
                                   ...
                                   {.depth = bottomDepth,
                                    .soundSpeed = 1500.0,
                                    .density = 1000.0}}),
                              broadband::BoundaryModel::vacuum(
                                  broadband::BoundaryGeometry::curvilinear(
                                      ...
                                      0.0,
                                      broadband::BoundaryOrientation::Upper)),
                              broadband::BoundaryModel::rigid(
                                  broadband::BoundaryGeometry::curvilinear(
                                      ...
                                      130.0,
                                      broadband::BoundaryOrientation::Lower)))
                        : broadband::Environment(
                              broadband::SoundSpeedProfile(
                                  {{.depth = 0.0,
                                    .soundSpeed = 1500.0,
                                   ...
                                   {.depth = bottomDepth,
                                    ...
                                    .density = 1000.0}}),
                              broadband::BoundaryModel::vacuum(0.0),
                              broadband::BoundaryModel::rigid(bottomDepth))));
  ...
  for (std::size_t index = 0U; index < path.points.size(); ++index) {
    const broadband::RayState& point = path.points[index];
    broadband::StepQuadrature step;
