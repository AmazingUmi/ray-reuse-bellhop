...
#include "broadband/field/pressure_scaling.hpp"
...
using broadband::AcousticMaterial;
...
using broadband::GeometricGaussianDiagnosticRequest;
using broadband::GeometricGaussianInfluence;
using broadband::GeometricGaussianWidthBranch;
...
[[nodiscard]] SimulationCase makeMunkGaussianCase(
  ...
  return SimulationCase(
      broadband::test::makeMunkEnvironment(
...
void checkDivergentPrefixes(Context& context,
                            const broadband::RayPathCache& cache,
  ...
  std::cout << "range-geometric-gaussian-parity " << label
            << ": rays=" << cache.size()
...
void checkSigmaBranchDivergence(Context& context,
  ...
  for (const std::size_t rangeIndex : {1U, 2U, 3U, 4U, 5U}) {
    for (const std::size_t depthIndex : {1U, 2U, 3U}) {
      requests.push_back(GeometricGaussianDiagnosticRequest{
          .receiverRangeIndex = rangeIndex, .receiverDepthIndex = depthIndex});
    }
  }
  ...
  for (const broadband::RayPath& path : cache.paths()) {
    ...
    for (const GeometricGaussianDiagnosticRequest& request : requests) {
      ...
      for (std::size_t frequencyIndex = 0U; frequencyIndex < frequencies.size();
        ...
        const broadband::RayFrequencyState state =
            projector.project(path, frequencies[frequencyIndex], 1.0);
        const std::optional<broadband::GeometricGaussianDiagnostic> diagnostic =
            kernel.accumulate(scratch[frequencyIndex], path, state,
                              launchAngleStep, request);
      ...
      const bool anyEvaluated = std::any_of(evaluated.begin(), evaluated.end(),
                                            [](bool value) { return value; });
      const bool allEvaluated = std::all_of(evaluated.begin(), evaluated.end(),
                                            [](bool value) { return value; });
...
[[nodiscard]] IntensityWorkspace legacyRawIntensity(
  ...
  const broadband::LaunchFanPlan& launchFan = simulation.launchFanPlan();
  const broadband::GeometrySspEvaluator soundSpeedProfile(
      simulation.environment().soundSpeedProfile());
  const broadband::SoundSpeedSample sourceSample = soundSpeedProfile.evaluate(
      broadband::Vec2{.range = 0.0, .depth = source.depth}, 0U);
  ...
  for (const broadband::RayPath& path : cache.paths()) {
    ...
    const double projectedSourceAmplitude =
        broadband::usesLloydMirror(simulation.runMode())
            ? broadband::semiCoherentProjectedSourceAmplitude(
    ...
    const broadband::RayFrequencyState frequencyState =
...
void testParityLevels(Context& context, const SimulationCase& simulation,
  ...
  const std::vector<double>& frequencies = simulation.frequencies().values();
  const broadband::Source& source = simulation.sources().front();
  const broadband::LaunchFanPlan& launchFan = simulation.launchFanPlan();
  const broadband::GeometrySspEvaluator soundSpeedProfile(
      simulation.environment().soundSpeedProfile());
  const broadband::SoundSpeedSample sourceSample = soundSpeedProfile.evaluate(
  ...
  const broadband::RayFanTraceResult trace =
  ...
  if (coherentRunMode) {
    ...
    for (std::size_t frequencyIndex = 0U; frequencyIndex < frequencies.size();
      ...
      const FrequencyWorkspace fusedConverted =
          broadband::scaleGeometricIntensityToPressure(
  ...
  for (std::size_t frequencyIndex = 0U; frequencyIndex < frequencies.size();
    ...
    context.check(streamed[frequencyIndex].has_value() &&
                  ...
                  std::string(label) +
                      " Level C captured converted fused workspace "
...
void testMode(Context& context, SimulationRunMode runMode,
  ...
  testParityLevels(
      context, makeMunkGaussianCase(runMode), CartesianCervenySettings{},
      (std::string("fixture A (munk gaussian ") + modeLabel + ')').c_str());
  ...
  {
    const SimulationCase simulation = makeDivergentPrefixGaussianCase(runMode);
    const broadband::RayFanTraceResult trace =
        SingleFrequencySolver::traceSourceFan(simulation, 0U);
...
int main() {
  ...
  // broadband.reuse.range.solver; one assertion here keeps the parity
  ...
  if (context.failureCount() != 0) {
    std::cerr << context.failureCount()
              << " range-geometric-gaussian-parity assertion(s) failed\n";
    return 1;
  }
  std::cout << "All Bellhop Broadband range-geometric-gaussian-parity tests "
               "passed\n";
  return 0;
}
