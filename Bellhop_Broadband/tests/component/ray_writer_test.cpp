...
#include "broadband/io/ray_prefix_writer.hpp"
...
using broadband::readSourceBeamPattern;
...
void testDirectionalPattern(Context& context) {
  ...
  const std::filesystem::path sbp =
      std::filesystem::temp_directory_path() / "broadband_rrb2_pattern.sbp";
  {
...
void testActiveTerminalPrefix(Context& context) {
  ...
  context.check(
      broadband::ray_output_detail::terminalPrefixPointCount(path, state) == 3U,
...
void testExplicitFrequencyAndMultiFrequencyRejection(Context& context) {
  ...
  const std::filesystem::path output =
      std::filesystem::temp_directory_path() / "broadband_rrb2_writer.ray";
  std::error_code ignored;
...
int main() {
  ...
  std::cout << "All Broadband RR-B2 ray writer tests passed\n";
  return 0;
}
