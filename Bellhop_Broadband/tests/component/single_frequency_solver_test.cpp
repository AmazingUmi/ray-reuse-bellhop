...
using broadband::semiCoherentLloydMirrorFactor;
...
void testPerSourceTraceProducesIndependentFrozenCaches(Context& context) {
  ...
  for (std::size_t sourceIndex = 0U; sourceIndex < sourceTraces.size();
       ++sourceIndex) {
    const broadband::RayFanTraceResult& trace = sourceTraces[sourceIndex];
    fansComplete =
  ...
  const std::vector<broadband::RayFanTraceResult> repeatTraces =
      SingleFrequencySolver::traceAllSourceFans(simulation);
  ...
  const broadband::RayFanTraceResult firstSourceTrace =
...
void testSingleSourcePerSourceTraceEquivalence(Context& context) {
  ...
  const broadband::RayFanTraceResult legacy =
      SingleFrequencySolver::traceRayFan(simulation);
  const std::vector<broadband::RayFanTraceResult> perSource =
      SingleFrequencySolver::traceAllSourceFans(simulation);
  context.check(
  ...
  for (const std::size_t workerCount : {1U, 2U, 4U, 8U}) {
    const broadband::RayFanTraceResult parallel =
        SingleFrequencySolver::traceRayFan(
            simulation,
            broadband::RayFanTraceSettings{.workerCount = workerCount});
    context.check(
  ...
  context.expectThrows<ValidationError>(
      [&] {
        static_cast<void>(SingleFrequencySolver::traceRayFan(
            simulation, broadband::RayFanTraceSettings{.workerCount = 0U}));
      },
...
broadband::ParsedEnvironment parseSspSmokeCase(char sspOption) {
  std::istringstream input(renderSspSmokeCase(sspOption));
  return broadband::EnvironmentParser::parse(input, "ssp_smoke.env");
}

void testSplineEnvironmentSolverSmoke(Context& context) {
  const broadband::ParsedEnvironment splineParsed = parseSspSmokeCase('S');
  ...
  const broadband::Vec2 midDepthSample{.range = 0.0, .depth = 250.0};
  const broadband::GeometrySspEvaluator splineEvaluator(
  ...
  const broadband::ParsedEnvironment cParsedCase = parseSspSmokeCase('C');
  const SimulationCase& cSimulation = cParsedCase.simulationCase;
  const broadband::GeometrySspEvaluator cEvaluator(
  ...
  const broadband::RayFanTraceResult splineTrace =
      SingleFrequencySolver::traceRayFan(splineSimulation);
  const broadband::RayFanTraceResult cTrace =
...
int main() {
  ...
  std::cout << "All Bellhop Broadband single-frequency-solver tests passed\n";
  return 0;
}
