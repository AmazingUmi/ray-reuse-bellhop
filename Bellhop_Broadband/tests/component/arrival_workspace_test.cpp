...
ArrivalCandidate candidate(double amplitude, double phase, double delay,
  ...
  return ArrivalCandidate{
      amplitude, phase, {delay, 0.0}, sourceAngle, receiverAngle, 1, 2};
}
...
void testMergeAndCusp(Context& context) {
  ...
  merged.addCandidate(100.0, candidate(2.0, 0.2, 1.0 + 1.0e-8, 9.0, 10.0), 0U,
                      0U);

  const Arrival record = merged.arrivalsAt(0U, 0U).front();
  context.check(merged.candidateCount() == 2U && merged.appendCount() == 1U &&
  ...
  context.check(
      cusp.candidateCount() == 2U && cusp.mergeCount() == 0U &&
...
void testNoMergeAndEncounterOrder(Context& context) {
  ...
  workspace.addCandidate(100.0, candidate(2.0, static_cast<double>(0.05F), 1.0),
                         0U, 0U);
  workspace.addCandidate(100.0, candidate(3.0, 0.0, 1.0 + 1.0e-8), 0U, 0U);

  const auto arrivals = workspace.arrivalsAt(0U, 0U);
  context.check(
      workspace.mergeCount() == 0U && arrivals.size() == 3U,
      "phase tolerance is strict and grouping compares only the last arrival");
  context.check(
      arrivals[0].amplitude == 1.0F && arrivals[1].amplitude == 2.0F &&
...
void testCapacityReplacement(Context& context) {
  ...
  context.check(workspace.saturatedCellCount() == 1U &&
                ...
                "capacity replacement uses candidate double strength and the "
                "first weakest slot");
...
int main() {
  ...
  std::cout << "All Bellhop Broadband arrival-workspace tests passed\n";
  return 0;
}
