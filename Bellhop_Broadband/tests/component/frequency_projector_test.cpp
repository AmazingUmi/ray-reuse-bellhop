...
#include "broadband/acoustics/attenuation.hpp"
#include "broadband/error.hpp"
#include "broadband/model/c_linear_frequency_ssp.hpp"
...
using broadband::CLinearFrequencySsp;
using broadband::convertAttenuation;
...
using broadband::RawAttenuation;
...
using broadband::ReflectionEvent;
...
RayPath makeAcousticReflectionPath() {
  const auto& fixture = broadband::test::kHalfSpaceCoefficientFixtures[1U];
...
void testMunkReflectionOracle(Context& context) {
  const Environment environment = broadband::test::makeMunkEnvironment();
  const RayPath path =
      GeometryTracer(environment, broadband::test::makeMunkIntegratorSettings())
          .trace(Source{.depth = 1000.0},
                 broadband::test::kMunkExtremeLaunchAngle);
  const RayFrequencyState state =
  ...
  const auto& fixture = broadband::test::kHalfSpaceCoefficientFixtures.back();
...
void testQuadrilateralFrozenPathProjection(Context& context) {
  const Environment environment(
      SoundSpeedProfile({{.depth = 0.0,
                        ...
                        std::make_shared<const broadband::QuadrilateralSspGrid>(
                            broadband::QuadrilateralSspGrid{
  ...
  context.check(
      path.terminationReason == broadband::RayTerminationReason::ExitedDomain,
...
int main() {
  ...
  std::cout << "All Bellhop Broadband frequency-projector tests passed\n";
  return 0;
}
