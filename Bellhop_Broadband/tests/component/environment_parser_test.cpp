...
const std::filesystem::path kCasesRoot =
    std::filesystem::path(BROADBAND_WORKSPACE_ROOT) /
...
void testCurvilinearBoundaryHeaders(Context& context) {
  ...
  const broadband::BoundaryGeometry& seabed =
      curvilinearEnvironment.seabed().geometry();
  const broadband::BoundaryGeometrySample interior =
  ...
  context.check(interior.curvature != 0.0,
                ...
                "curvature");
  context.check(std::abs(broadband::norm(interior.tangent) - 1.0) > 1.0e-9,
  ...
  const broadband::BoundaryGeometrySample extension =
  ...
  context.check(
      parsedCanonical.simulationCase.environment()
          ...
          parsedCanonical.simulationCase.environment()
                  ...
                  .geometry()
                  .interpolationKind() ==
              broadband::BoundaryInterpolationKind::PiecewiseLinear,
...
void testVolumeAttenuationParsing(Context& context) {
  ...
  const auto& fgParameters = std::get<broadband::FrancoisGarrisonParameters>(
...
void testPrtVolumeAttenuationMarkers(Context& context) {
  ...
  const auto checkMarkers = [&](const std::string& name,
    ...
    const std::filesystem::path directory =
        std::filesystem::temp_directory_path() / ("broadband_h02_" + name);
...
void testIrregularReceiverLayouts(Context& context) {
  ...
  context.check(plain.simulationCase.receivers().layout() ==
                        broadband::ReceiverGridLayout::Rectilinear &&
  ...
  for (const auto& [runType, expectedMode] :
    ...
    context.check(
        parsed.simulationCase.runMode() == expectedMode &&
            receivers.layout() == broadband::ReceiverGridLayout::Irregular &&
...
void testUnsupportedAndMalformedInput(Context& context) {
  ...
  context.check(
      n2Parsed.simulationCase.environment()
              .soundSpeedProfile()
              .interpolationKind() == broadband::SspInterpolationKind::N2Linear,
...
void testSspInterpolationKinds(Context& context) {
  ...
  context.check(
      pchipParsed.simulationCase.environment()
              .soundSpeedProfile()
              .interpolationKind() == broadband::SspInterpolationKind::Pchip,
  ...
  context.check(
      cParsed.simulationCase.environment()
              .soundSpeedProfile()
              .interpolationKind() == broadband::SspInterpolationKind::CLinear,
  ...
  const broadband::SoundSpeedProfile programmatic(
      {broadband::SoundSpeedPoint{
...
int main() {
  ...
  std::cout << "All Bellhop Broadband environment-parser tests passed\n";
  return 0;
}
