...
using broadband::FlatBoundaryReflection;
...
void testSeaSurfaceMirrorAndEvent(Context& context) {
  ...
  const FlatBoundaryReflection result = broadband::reflectAtFlatBoundary(
  ...
  context.checkNear(
      kSoundSpeed * broadband::norm(result.reflectedState.slowness), 1.0,
  ...
  context.checkNear(event.tangentSlowness,
                    broadband::dot(incident.slowness, {1.0, 0.0}), 1.0e-15,
                    "event stores tangent component");
  context.checkNear(event.normalSlowness,
                    broadband::dot(incident.slowness, {0.0, -1.0}), 1.0e-15,
...
void testSeabedDynamicJump(Context& context) {
  ...
  const double cnJump =
      -broadband::dot(gradient, reflectedRayNormal - incidentRayNormal);
  const double csJump = -broadband::dot(gradient, reflectedUnit - incidentUnit);
...
void testSurfaceSignAndCurvatureModes(Context& context) {
  ...
  const FlatBoundaryReflection standard = broadband::reflectAtFlatBoundary(
      incident, ReflectionBoundary::SeaSurface, geometry, 2U,
      BoundaryCurvatureMode::Standard);
  const FlatBoundaryReflection doubled = broadband::reflectAtFlatBoundary(
      incident, ReflectionBoundary::SeaSurface, geometry, 2U,
      BoundaryCurvatureMode::Double);
  const FlatBoundaryReflection zeroed = broadband::reflectAtFlatBoundary(
      incident, ReflectionBoundary::SeaSurface, geometry, 2U,
...
void testLegacyCurvilinearFrame(Context& context) {
  ...
  const broadband::BoundaryReflectionGeometry geometry{
  ...
  const FlatBoundaryReflection standard = broadband::reflectAtBoundary(
  ...
  context.check(
      kSoundSpeed * broadband::norm(standard.reflectedState.slowness) != 1.0,
...
void testInvalidGeometry(Context& context) {
  ...
  context.expectThrows<ValidationError>(
      [&] {
        (void)broadband::reflectAtFlatBoundary(
  ...
  const FlatBoundaryReflection overshoot = broadband::reflectAtFlatBoundary(
  ...
  const FlatBoundaryReflection drifted = broadband::reflectAtFlatBoundary(
