...
[[nodiscard]] SimulationCase makeMunkRayCenteredCase(
  ...
  return SimulationCase(
      ...
      broadband::BeamFamily::CervenyGaussian, fieldComponent,
...
[[nodiscard]] std::size_t gateAcceptedStepCount(const broadband::RayPath& path,
                                                std::size_t prefix) {
  std::size_t count = 0U;
...
[[nodiscard]] IntensityWorkspace legacyRawIntensity(
  ...
  const FrequencyProjector projector(simulation.environment());
  const broadband::RayCenteredCervenyInfluence kernel(
...
int main() {
  ...
  std::cout << "All Bellhop Broadband range-rc-parity tests passed\n";
  return 0;
}
