...
using broadband::reflectAtFlatBoundary;
...
bool finiteState(const RayState& state) {
  return broadband::isFinite(state.position) &&
         broadband::isFinite(state.slowness) &&
  ...
  for (const auto& step : path.steps) {
    context.check(
        std::isfinite(step.stepLength) && std::isfinite(step.startWeight) &&
            ...
            broadband::isFinite(step.midpoint),
...
void testSeaBoundariesReflect(Context& context) {
  ...
  context.check(
      !surface.events.empty() && surface.events.front().boundary ==
                                     broadband::ReflectionBoundary::SeaSurface,
  ...
  context.check(
      !seabed.events.empty() && seabed.events.front().boundary ==
                                    broadband::ReflectionBoundary::Seabed,
...
void checkN2ReflectionAgainstArrivalSideWaterSample(
  ...
  const broadband::ReflectionEvent& event = path.events.front();
  const broadband::SoundSpeedSample arrivalWater =
  ...
  const auto expected = reflectAtFlatBoundary(
      ...
      FlatBoundaryGeometry{
          ...
          .tangent = broadband::Vec2{1.0, 0.0},
          .outwardNormal = broadband::Vec2{0.0, outwardNormalDepth},
...
int main() {
  ...
  std::cout
      << "All Bellhop Broadband SSP-interface geometry-tracer tests passed\n";
  return 0;
}
