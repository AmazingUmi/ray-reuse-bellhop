...
#include "broadband/model/pchip_frequency_ssp.hpp"
#include "broadband/model/pchip_ssp.hpp"
...
void testPchipNonzeroCurvature(Context& context) {
  const auto munkEnvironment =
      broadband::test::makeMunkEnvironment(SspInterpolationKind::Pchip);
...
int main() {
  ...
  std::cout << "All Bellhop Broadband sound-speed evaluator tests passed\n";
  return 0;
}
