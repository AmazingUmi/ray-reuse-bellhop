...
//             ReuseRangeParaSolver::accumulateFrequencies (std::memcmp over
...
//             ReuseRangeParaSolver::solveStreaming) on the same SimulationCase.
...
[[nodiscard]] SimulationCase makeMunkSmallCase(
  ...
  return SimulationCase(
      ...
      broadband::SourceBeamPattern::omnidirectional(),
      broadband::SimulationRunMode::Coherent,
...
void checkDivergentPrefixes(Context& context,
  ...
  std::cout << "range-cc-parity " << label << ": rays=" << cache.size()
            << " divergent-prefix rays=" << divergentRays
            << " cutoff-truncated rays=" << truncatedRays << " example ray "
            << exampleRay << " prefix(f=" << frequencies.front()
            << ")=" << exampleLowPrefix << " prefix(f=" << frequencies.back()
            << ")=" << exampleHighPrefix << '\n';
...
void testParityLevels(Context& context, const SimulationCase& simulation,
  ...
  context.check(
      ...
      std::string(label) +
          " Level B result carries every frequency "
...
int main() {
  ...
  testParityLevels(
      context,
      makeMunkSmallCase(FrequencyGrid({50.0, 250.0}), BeamWidthMode::Wkb),
  ...
  std::cout << "All Bellhop Broadband range-cc-parity tests passed\n";
  return 0;
}
