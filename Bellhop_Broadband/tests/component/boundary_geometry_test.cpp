#include "broadband/model/boundary_geometry.hpp"
...
using broadband::BoundaryGeometrySample;
using broadband::BoundaryInterpolationKind;
...
void testCurvilinearNodeFrameAndCurvature(Context& context) {
  ...
  const double fraction =
      broadband::dot(midpoint - kNodes[0U], first.tangent) / first.length;
  ...
  context.checkNear(sample.tangent.depth, expectedTangent.depth, 1.0e-12,
                    ...
                    "0.5-average node interpolation");
  context.check(std::abs(broadband::norm(sample.tangent) - 1.0) > 1.0e-6,
                "interpolated curvilinear reflection frame is not unit "
                "length");
  context.checkNear(broadband::dot(sample.tangent, sample.outwardNormal), 0.0,
  ...
  context.check(chord.curvature == 0.0 &&
                    std::abs(broadband::norm(chord.tangent) - 1.0) <= 1.0e-15,
...
void testPiecewiseAndFlatReflectionFramesUnchanged(Context& context) {
  ...
  context.expectThrows<broadband::ValidationError>(
      [&] {
...
int main() {
  ...
  std::cout << "All Bellhop Broadband boundary-geometry tests passed\n";
  return 0;
}
