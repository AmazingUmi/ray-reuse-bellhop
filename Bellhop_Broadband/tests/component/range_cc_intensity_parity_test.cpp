...
#include "broadband/field/beam_epsilon.hpp"
...
using broadband::CartesianCervenyInfluence;
...
[[nodiscard]] SimulationCase makeMunkIntensityCase(
  ...
  return SimulationCase(
      ...
      broadband::FieldComponent::Pressure,
      broadband::BoundaryCurvatureMode::Standard, widthMode,
...
void checkDivergentPrefixes(Context& context,
  ...
  std::cout << "range-cc-intensity-parity " << label
            << ": rays=" << cache.size()
...
[[nodiscard]] IntensityWorkspace legacyRawIntensity(
  ...
  for (const broadband::RayPath& path : cache.paths()) {
    ...
    const broadband::BeamEpsilon epsilon = broadband::pickBeamEpsilon(
...
void testIntensityParityLevels(Context& context,
  ...
  context.check(
      ...
      std::string(label) +
          " Level B result carries every frequency and the "
  ...
  for (std::size_t frequencyIndex = 0U; frequencyIndex < frequencies.size();
    ...
    context.check(levelBConverted.equal, std::string(label) +
...
void testMode(Context& context, SimulationRunMode runMode,
  ...
  testIntensityParityLevels(
      context, makeMunkIntensityCase(runMode), CartesianCervenySettings{},
      (std::string("fixture A (munk CC ") + modeLabel + ", 3 images)").c_str());
  ...
  {
    const SimulationCase simulation = makeDivergentPrefixIntensityCase(runMode);
    const broadband::RayFanTraceResult trace =
        SingleFrequencySolver::traceSourceFan(simulation, 0U);
...
int main() {
  ...
  if (context.failureCount() != 0) {
    std::cerr << context.failureCount()
              << " range-cc-intensity-parity assertion(s) failed\n";
    return 1;
  }
  std::cout << "All Bellhop Broadband range-cc-intensity-parity tests passed\n";
  return 0;
}
