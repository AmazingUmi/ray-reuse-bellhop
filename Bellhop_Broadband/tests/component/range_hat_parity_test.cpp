...
using broadband::GeometricHatInfluence;
...
[[nodiscard]] const char* coordinateLabel(CervenyCoordinateSystem coordinates) {
...
void checkDivergentPrefixes(Context& context,
  ...
  std::cout << "range-hat-parity " << label << ": rays=" << cache.size()
...
void testMode(Context& context, SimulationRunMode runMode,
  ...
  for (const CervenyCoordinateSystem coordinates :
    ...
    testParityLevels(context, makeMunkHatCase(runMode, coordinates),
    ...
    // fixture fails loudly instead of passing vacuously.
    {
      ...
      checkDivergentPrefixes(context, trace.cache, simulation,
                             (std::string("fixture B (lossy halfspace, hat ") +
...
int main() {
  ...
  context.check(
      supportsReuseRangePara(makeMunkHatCase(
          SimulationRunMode::Coherent, CervenyCoordinateSystem::Cartesian)) &&
          supportsReuseRangePara(
              makeMunkHatCase(SimulationRunMode::SemiCoherent,
                              CervenyCoordinateSystem::RayCentered)),
  ...
  std::cout << "All Bellhop Broadband range-hat-parity tests passed\n";
  return 0;
}
