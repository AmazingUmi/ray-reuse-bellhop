...
using broadband::ArrivalSolver;
using broadband::ArrivalSolverStatistics;
...
using broadband::EigenraySolverStatistics;
using broadband::EigenraySourceHits;
...
bool sameArrival(const broadband::Arrival& left,
                 const broadband::Arrival& right) {
...
using ArrivalCells = std::vector<std::vector<broadband::Arrival>>;

ArrivalCells snapshotArrivals(const broadband::ArrivalWorkspace& workspace) {
...
int main() {
  ...
  std::cout << "All Bellhop Broadband multi-source product tests passed\n";
  return 0;
}
