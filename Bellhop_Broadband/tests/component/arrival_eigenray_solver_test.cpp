...
void testEmptyProducts(Context& context) {
  ...
  const auto asciiPath = base / "broadband_b3_empty.arr";
  const auto binaryPath = base / "broadband_b3_empty_binary.arr";
  const auto eigenrayPath = base / "broadband_b3_empty.e";
...
void testProductValidation(Context& context) {
  ...
  const auto rejectedPath =
      std::filesystem::temp_directory_path() / "broadband_b3_rejected.arr";
...
int main() {
  ...
  std::cout << "All Bellhop Broadband arrival/eigenray solver tests passed\n";
  return 0;
}
