...
#include "broadband/ray/flat_boundary_reflection.hpp"
#include "broadband/ray/geometry_tracer.hpp"
...
using broadband::FlatBoundaryGeometry;
using broadband::GeometryTracer;
...
using broadband::ReflectionBoundary;
...
void checkEventPair(Context& context, const RayPath& path,
  ...
  context.checkNear(broadband::dot(incident.slowness, event.boundaryTangent),
                    broadband::dot(reflected.slowness, event.boundaryTangent),
                    1.0e-15, "flat reflection preserves tangent slowness");
  context.checkNear(broadband::dot(incident.slowness, event.outwardNormal),
                    -broadband::dot(reflected.slowness, event.outwardNormal),
                    1.0e-15,
...
void testSingleSeaSurfaceReflectionAndArrivalGradient(Context& context) {
  ...
  const auto expected = broadband::reflectAtFlatBoundary(
...
void testTracerPropagatesCurvatureMode(Context& context) {
  ...
  context.check(standard.events.size() == 1U && doubled.events.size() == 1U &&
  ...
  for (std::size_t basis = 0U; basis < 2U; ++basis) {
    ...
    const double doubledJump = doubled.points[reflectedIndex].dynamicP[basis] -
...
void testCurvilinearMultiBounceMatchesFortran(Context& context) {
  ...
  const GeometryTracer tracer(makeCurvilinearSurfaceAndBottomEnvironment(),
  ...
  for (const auto& event : path.events) {
    ...
    context.check(broadband::norm(event.boundaryTangent) < 1.0,
  ...
  if (path.events.size() >= 2U) {
    ...
    context.checkNear(top.reflectedSlowness.range, 0.0006441655711190538,
                      1.0e-18, "first reflected range slowness matches oracle");
    context.checkNear(top.reflectedSlowness.depth, 0.00017164054349210126,
                      1.0e-18, "first reflected depth slowness matches oracle");
    ...
    context.checkNear(bottom.boundaryTangent.range, 0.9991572645683603, 1.0e-15,
...
void testCurvilinearTracerPropagatesCurvatureMode(Context& context) {
  constexpr double launchAngle = -2.438888037721334e-1;
  const Environment environment = makeCurvilinearSurfaceAndBottomEnvironment();
  ...
  const std::size_t zeroedPoint = zeroed.events.front().reflectedRayPointIndex;
...
void testCurvilinearTwoPointsOutsideTermination(Context& context) {
  ...
  const broadband::BoundaryGeometry top =
      broadband::BoundaryGeometry::curvilinear(
          {{.range = 0.0, .depth = 4.0},
           {.range = 450.0, .depth = 15.0},
           {.range = 1050.0, .depth = 2.0},
           {.range = 1650.0, .depth = 19.0},
