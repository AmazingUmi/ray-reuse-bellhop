...
void testMunkOracleRegression(Context& context) {
  const GeometryTracer tracer(broadband::test::makeMunkEnvironment(),
                              broadband::test::makeMunkIntegratorSettings());
  const auto path = tracer.trace(Source{.depth = 1000.0},
                                 broadband::test::kMunkOracleLaunchAngle);
...
void testMunkBoundaryReflectionRegression(Context& context) {
  ...
  constexpr std::array expectedTraces{
      ExpectedTrace{.launchAngle = -broadband::test::kMunkExtremeLaunchAngle,
                    .pointCount = 547U,
                    .stepCount = 539U,
                    .topBounceCount = 4U,
