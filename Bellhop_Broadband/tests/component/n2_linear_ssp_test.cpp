#include "broadband/model/n2_linear_ssp.hpp"
...
#include "broadband/model/n2_linear_frequency_ssp.hpp"
#include "support/test_harness.hpp"

namespace {

using broadband::AttenuationUnit;
using broadband::FrancoisGarrisonParameters;
using broadband::N2LinearFrequencySsp;
using broadband::N2LinearSsp;
using broadband::SoundSpeedProfile;
using broadband::SspInterpolationKind;
...
void checkRealOracle(Context& context, const N2LinearSsp& profile, double depth,
                     std::size_t segment, double expectedSoundSpeed,
                     double expectedGradient, double expectedCurvature,
                     double expectedDensity, const char* description) {
  const auto sample =
  ...
  context.checkNear(sample.soundSpeedGradient.depth, expectedGradient, 3.0e-15,
                    std::string(description) + " gradient");
  context.checkNear(sample.soundSpeedHessian.depthDepth, expectedCurvature,
                    3.0e-18, std::string(description) + " curvature");
...
void testRealFortranOracle(Context& context) {
  ...
  checkRealOracle(context, profile, 50.0, 0U, 1547.5821125259863,
                  0.99740219310406519, 0.00192844914675006224, 1050.0,
                  "first midpoint");
  checkRealOracle(context, profile, 100.0, 0U, 1600.0, 1.10222222222222155,
                  0.00227792592592592326, 1100.0, "node from left");
  checkRealOracle(context, profile, 100.0, 1U, 1600.0, -2.44897959183673342,
                  0.0112453144523115226, 1100.0, "node from right");
  checkRealOracle(context, profile, 150.0, 1U, 1490.02583573253605,
                  -1.97791040141486962, 0.00787663434191761790,
                  1200.00000000000023, "second midpoint");
}
...
void testNodeAndExtrapolationSemantics(Context& context) {
  ...
  const auto above = profile.evaluate(Vec2{.range = 0.0, .depth = -20.0}, 1U);
  const auto below = profile.evaluate(Vec2{.range = 0.0, .depth = 220.0}, 0U);
...
void testFrequencyComplexOracle(Context& context) {
  ...
  context.checkNear(sample.imaginarySoundSpeed, 6.87988934309839983, 4.0e-14,
                    "complex N2 interpolation matches Fortran imaginary part");
  context.checkNear(sample.soundSpeedGradient.depth, -0.0397683421458134914,
                    4.0e-16,
                    "complex N2 gradient follows Fortran's real slope path");
  context.checkNear(sample.soundSpeedHessian.depthDepth, 3.18444961960798548e-6,
                    4.0e-19, "complex N2 curvature matches Fortran");
...
void testVolumePaths(Context& context) {
  ...
  const auto layers =
      std::make_shared<const broadband::BiologicalAttenuationLayers>(
          broadband::BiologicalAttenuationLayers{
              {.minimumDepth = 0.0,
               .maximumDepth = 100.0,
               .resonanceFrequency = 1000.0,
               .qualityFactor = 2.0,
               .attenuationCoefficientDecibelsPerKilometer = 10.0}});
  const VolumeAttenuation bioModel{.model = VolumeAttenuationModel::Biological,
                                   .parameters = layers};
  ...
  const SoundSpeedProfile legacyThorp(std::move(legacyPoints),
                                      SspInterpolationKind::N2Linear);
  context.check(N2LinearFrequencySsp(source, 1000.0, thorp)
  ...
  const auto low = N2LinearFrequencySsp(source, 500.0, bioModel)
                       .evaluate(Vec2{.range = 0.0, .depth = 50.0}, 0U);
  static_cast<void>(N2LinearFrequencySsp(source, 2000.0, bioModel)
                        .evaluate(Vec2{.range = 0.0, .depth = 50.0}, 0U));
  const auto repeated = N2LinearFrequencySsp(source, 500.0, bioModel)
                            .evaluate(Vec2{.range = 0.0, .depth = 50.0}, 0U);
...
void testValidation(Context& context) {
  ...
  context.expectThrows<ValidationError>(
      [&profile] {
        static_cast<void>(
            profile.evaluateAtSegment(Vec2{.range = 0.0, .depth = 150.0}, 0U));
      },
  ...
  const N2LinearSsp decreasing(SoundSpeedProfile(
  ...
  context.expectThrows<ValidationError>(
      [&decreasing] {
        static_cast<void>(
            decreasing.evaluate(Vec2{.range = 0.0, .depth = -730.0}, 1U));
      },
...
int main() {
  ...
  std::cout << "All Bellhop Broadband N2-linear SSP tests passed\n";
  return 0;
}
