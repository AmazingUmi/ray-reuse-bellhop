#include "broadband/ray/ray_stepper.hpp"
...
#include "broadband/model/c_linear_ssp.hpp"
#include "broadband/model/cubic_spline_frequency_ssp.hpp"
#include "broadband/model/cubic_spline_ssp.hpp"
#include "broadband/model/environment.hpp"
#include "broadband/model/sound_speed_evaluator.hpp"
#include "broadband/model/sound_speed_types.hpp"
#include "broadband/ray/ray_equations.hpp"
#include "support/test_harness.hpp"

namespace {

using broadband::CLinearSsp;
using broadband::CubicSplineFrequencySsp;
using broadband::CubicSplineSsp;
using broadband::GeometrySspEvaluator;
using broadband::QuadrilateralSspGrid;
using broadband::RayState;
using broadband::SharedQuadrilateralSspGrid;
using broadband::SoundSpeedHessian;
using broadband::SoundSpeedPoint;
using broadband::SoundSpeedProfile;
using broadband::SoundSpeedSample;
using broadband::SspGradientContinuity;
using broadband::SspInterpolationKind;
using broadband::StepLimitPhase;
using broadband::StepLimitRequest;
using broadband::stepRay;
...
void testCLinearSegmentJump(Context& context) {
  ...
  const double correction =
      incidenceTangent *
      (2.0 * broadband::dot(gradientJump, rayNormal) -
       incidenceTangent *
           broadband::dot(gradientJump, result.endState.slowness)) /
...
void testN2SegmentCrossingAppliesGradientJump(Context& context) {
  ...
  const auto jumpCorrection = [&](const GeometrySspEvaluator& ssp,
                                  const auto& result) {
    const broadband::SoundSpeedSample initialSample =
        ssp.evaluateAtSegment(initial.position, 0U);
    const broadband::SoundSpeedSample endSample =
...
void testSplineGradientContinuityAvoidsNodeJump(Context& context) {
  ...
  const double initialCurvature =
      broadband::soundSpeedNormalSecondDerivativeOverSquaredSpeed(
  ...
  const double erroneousJumpCorrection =
      incidenceTangent *
      (2.0 * broadband::dot(gradientDifference, rayNormal) -
       incidenceTangent *
           broadband::dot(gradientDifference, result.endState.slowness)) /
      endSample.soundSpeed;
  context.check(
...
double jumpCorrection(const GeometrySspEvaluator& ssp, const RayState& initial,
  ...
  const double normalGradientJump = broadband::dot(gradientJump, rayNormal);
  const double tangentGradientJump = broadband::dot(gradientJump, end.slowness);
  const double incidenceTangent =
...
int main() {
  ...
  std::cout << "All Bellhop Broadband ray-stepper tests passed\n";
  return 0;
}
