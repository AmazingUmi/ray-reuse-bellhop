...
[[nodiscard]] SimulationCase makeCase(
  ...
  return SimulationCase(
      ...
      broadband::BeamWidthMode::MinimumWidth, coordinates);
}
...
void checkParity(Context& context, const SimulationCase& simulation,
  ...
  const broadband::RayFanTraceResult trace =
      broadband::SingleFrequencySolver::traceSourceFan(simulation, sourceIndex);
  const std::uint64_t fingerprint = trace.cache.contentFingerprint();

  std::vector<broadband::ArrivalWorkspace> legacy;
  ...
  const broadband::FusedArrivalAccumulationResult fusedOne =
      broadband::ReuseRangeParaSolver::accumulateArrivalFrequencies(
          simulation, trace.cache, sourceIndex, {},
          {.requestedRangeWorkers = 1U});
  const broadband::FusedArrivalAccumulationResult fusedFour =
      broadband::ReuseRangeParaSolver::accumulateArrivalFrequencies(
          simulation, trace.cache, sourceIndex, {},
    ...
    for (std::size_t frequencyIndex = 0U;
      ...
      for (std::size_t depthIndex = 0U;
        ...
        for (std::size_t rangeIndex = 0U;
          ...
          const std::span<const broadband::Arrival> expected =
              legacy[frequencyIndex].arrivalsAt(depthIndex, rangeIndex);
          const std::span<const broadband::Arrival> observed =
              actual.arrivalsAt(depthIndex, rangeIndex);
          const std::span<const broadband::Arrival> serialFused =
...
int main() {
  ...
  std::cout << "All Bellhop Broadband fused-arrival-parity tests passed\n";
  return 0;
}
