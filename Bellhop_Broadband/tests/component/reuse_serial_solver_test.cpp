...
void checkPressureEqual(Context& context,
                        ...
                        const broadband::SingleFrequencyResult& expected,
                        const char* message) {
...
void testEnvironmentPayloadIsExternalToFrozenCache(Context& context) {
  for (const auto& parameterPair :
    ...
    const broadband::RayFanTraceResult trace =
        broadband::SingleFrequencySolver::traceRayFan(firstSimulation);
    const broadband::RayFanTraceResult changedTrace =
        broadband::SingleFrequencySolver::traceRayFan(changedSimulation);
    ...
    const broadband::SingleFrequencyResult firstLow =
        broadband::SingleFrequencySolver::solveFrequencyFromCache(
    ...
    const broadband::SingleFrequencyResult changedLow =
        broadband::SingleFrequencySolver::solveFrequencyFromCache(
...
void testMunkSplineFrozenGeometryAnchor(Context& context) {
  ...
  const SimulationCase simulation(
      ...
      broadband::test::makeMunkIntegratorSettings());

  const broadband::RayFanTraceResult trace =
      broadband::SingleFrequencySolver::traceRayFan(simulation);
...
int main() {
  ...
  std::cout << "All Bellhop Broadband reuse-serial-solver tests passed\n";
  return 0;
}
