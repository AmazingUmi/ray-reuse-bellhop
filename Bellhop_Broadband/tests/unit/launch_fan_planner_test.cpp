#include "broadband/model/launch_fan_planner.hpp"
...
void testFortranDegreeSubtabulationBits(Context& context) {
  LaunchFanPlanningInput input{
      ...
      .inputDegreeBounds =
          broadband::LaunchAngleDegreeBounds{.minimum = -89.0, .maximum = 89.0},
  };
  const LaunchFanPlan plan = LaunchFanPlanner::plan(input);
...
void testInvalidInputs(Context& context) {
  ...
  input = directCaseInput();
  input.inputDegreeBounds = broadband::LaunchAngleDegreeBounds{
  ...
  input.inputDegreeBounds =
      broadband::LaunchAngleDegreeBounds{.minimum = 5.0, .maximum = -5.0};
  expectInvalid(input, "a reversed parser degree interval is rejected");

  input = directCaseInput();
  input.inputDegreeBounds =
      broadband::LaunchAngleDegreeBounds{.minimum = -6.0, .maximum = 5.0};
  expectInvalid(input,
...
int main() {
  ...
  std::cout << "All Bellhop Broadband launch fan planner tests passed\n";
  return 0;
}
