# Bellhop Broadband 产品实现

本目录是独立的 C++20 多频 Bellhop 实现，产品名为 **Bellhop Broadband**
（可执行程序 `bellhop_broadband`；仓库名保留历史 `ray-reuse-bellhop`）。
...
```bash
uv run cmake --preset debug -S Bellhop_Broadband
uv run cmake --build Bellhop_Broadband/build/debug --parallel
uv run ctest --test-dir Bellhop_Broadband/build/debug --output-on-failure
...
```bash
BROADBAND_BUILD_JOBS=4 uv run bash Bellhop_Broadband/scripts/quality_gate.sh
```
...
```bash
BROADBAND_BUILD_JOBS=4 uv run bash Bellhop_Broadband/scripts/engineering_gate.sh
```
...
```bash
Bellhop_Broadband/scripts/model_matrix_gate.sh
Bellhop_Broadband/scripts/intermediate_state_gate.sh
Bellhop_Broadband/scripts/single_thread_microbenchmark.sh
```
...
```bash
Bellhop_Broadband/build/release/bellhop_broadband <file-root>
```
