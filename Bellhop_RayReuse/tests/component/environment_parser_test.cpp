...
void testFrequencyOverride(Context& context) {
  ...
  const ParsedEnvironment parsed =
      EnvironmentParser::parse(
          input, "broadband_direct.env",
          std::vector<double>{50.0, 250.0});

  context.check(
      parsed.simulationCase.frequencies().values() ==
          std::vector<double>({50.0, 250.0}) &&
          parsed.simulationCase.frequencies().designFrequency() ==
              250.0 &&
          parsed.simulationCase.launchFanPlan().designFrequency ==
              250.0,
...
void testUnsupportedAndMalformedInput(Context& context) {
  ...
  context.expectThrows<ValidationError>(
      [&] {
        ...
        static_cast<void>(
            parseText(contents, "incoherent.env"));
      },
  ...
  context.expectThrows<ValidationError>(
      [&] {
        ...
        static_cast<void>(
            parseText(contents, "filling_beam.env"));
      },
  ...
  context.expectThrows<ValidationError>(
      [&] {
        std::string contents = direct;
        replaceFirst(
            contents, "1000.0  1600.0  0.0  1.8",
