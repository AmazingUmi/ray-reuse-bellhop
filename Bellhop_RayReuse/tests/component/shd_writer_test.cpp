...
static_assert(
    !std::is_copy_constructible_v<ShdFrequencyWriter>);
static_assert(
    !std::is_copy_assignable_v<ShdFrequencyWriter>);
static_assert(
    std::is_nothrow_move_constructible_v<ShdFrequencyWriter>);
static_assert(
    std::is_nothrow_move_assignable_v<ShdFrequencyWriter>);
static_assert(
    std::is_nothrow_destructible_v<ShdFrequencyWriter>);
...
SimulationCase makeSimulation(
  ...
  return SimulationCase(
      ...
      receivers, FrequencyGrid(std::move(frequencies)),
      LaunchFan{
          .minimumAngle =
...
void testMultiFrequencyBinaryLayout(Context& context) {
  ...
  const std::filesystem::path path =
      directory.path() / "multi_frequency.shd";
  const ReceiverGrid receivers(
      {20.0, 30.0}, {1000.0, 2000.0});
  ...
  for (std::size_t frequencyIndex = 0U;
       frequencyIndex < workspaces.size(); ++frequencyIndex) {
    for (std::size_t depthIndex = 0U;
         depthIndex < receivers.depthCount(); ++depthIndex) {
      for (std::size_t rangeIndex = 0U;
           rangeIndex < receivers.rangeCount(); ++rangeIndex) {
        const double marker =
            100.0 * static_cast<double>(frequencyIndex) +
            10.0 * static_cast<double>(depthIndex) +
            static_cast<double>(rangeIndex) + 1.0;
        workspaces[frequencyIndex].at(depthIndex, rangeIndex) =
            {marker, -marker};
      }
    }
  }

  ShdWriter::writeFrequencies(
      path, "Two-frequency RayReuse fixture",
  ...
  context.check(
      loadInt32(bytes, 2U * recordBytes) == 2,
      "multi-frequency SHD header stores nfreq");
  context.checkNear(
      loadFloat64(bytes, 3U * recordBytes),
      50.0, 0.0, "multi-frequency SHD first frequency");
  context.checkNear(
      loadFloat64(bytes, 3U * recordBytes + 8U),
      100.0, 0.0, "multi-frequency SHD second frequency");
  ...
  context.check(
      loadFloat32(bytes, pressureStart) == 1.0F &&
          ...
          loadFloat32(bytes, pressureStart + 2U * recordBytes) ==
              101.0F &&
          loadFloat32(bytes, pressureStart + 3U * recordBytes) ==
              111.0F,
      "multi-frequency pressure records are frequency-major");
  context.check(
      loadFloat32(
          bytes, pressureStart + 2U * recordBytes + 12U) ==
          -102.0F,
  ...
  for (std::size_t frequencyIndex = 0U;
    ...
    for (std::size_t depthIndex = 0U;
      ...
      for (std::size_t rangeIndex = 0U;
        ...
        workspaces[frequencyIndex].at(
            depthIndex, rangeIndex) = {marker, -marker};
      }
    }
  }

  constexpr std::string_view title =
      "Out-of-order streaming fixture";
  ShdWriter::writeFrequencies(
      batchPath, title, simulation, workspaces);

  ShdFrequencyWriter writer(
      streamingPath, title, simulation);
  writer.writeFrequency(1U, workspaces[1U]);
  writer.writeFrequency(0U, workspaces[0U]);
  writer.finalize();

  context.check(
      readBytes(streamingPath) == readBytes(batchPath),
...
void testFrequencyAxisControlsRecordLength(Context& context) {
  ...
  const SimulationCase simulation =
      makeSimulation(receivers, frequencies);

  ShdWriter::writeFrequencies(
      path, "Wide frequency-axis fixture",
      simulation, workspaces);
  ...
  context.check(
      loadInt32(bytes, 0U) ==
          static_cast<std::int32_t>(recordWords),
      "SHD record length expands to contain the frequency axis");
  context.checkNear(
      loadFloat64(
          bytes, 3U * recordBytes + 20U * sizeof(double)),
      70.0, 0.0, "expanded SHD record contains the last frequency");
}

void testBinaryLayout(Context& context) {
  constexpr std::string_view titleText =
      "Synthetic RayReuse SHD fixture";
  TemporaryDirectory directory;
  const std::filesystem::path path =
...
void testValidation(Context& context) {
  ...
  context.expectThrows<ValidationError>(
      [&] {
        ...
        ShdWriter::writeFrequencies(
            directory.path() / "wrong_count.shd",
  ...
  context.expectThrows<ValidationError>(
      [&] {
        ...
        ShdWriter::writeFrequencies(
            directory.path() / "wrong_order.shd",
