...
std::vector<double> makeFrequencies(
    std::size_t count) {
  std::vector<double> frequencies;
  frequencies.reserve(count);
  for (std::size_t index = 0U; index < count; ++index) {
    frequencies.push_back(
        50.0 + 25.0 * static_cast<double>(index));
  }
  return frequencies;
}
...
StreamedParallelRun runParallel(
  ...
  StreamedParallelRun run{
      .workspaces =
          std::vector<
              std::optional<rayreuse::FrequencyWorkspace>>(
  ...
  run.statistics =
      ParallelRayReuseSolver::solveStreaming(
          ...
          [&run](
            ...
            run.workspaces.at(frequencyIndex).emplace(
                std::move(workspace));
          },
...
void checkWorkspaceEqual(
  ...
  context.check(
      actual.frequency() == expected.frequency() &&
          ...
          std::equal(
              actual.pressure().begin(),
              actual.pressure().end(),
              expected.pressure().begin(),
              expected.pressure().end()),
      message);
}

void testFrequencyCounts(Context& context) {
  for (const std::size_t frequencyCount :
       {1U, 2U, 16U}) {
    const SimulationCase simulation =
    ...
    context.check(
        parallel.statistics.tracePassCount == 1U &&
            serial.statistics.tracePassCount == 1U &&
            nonReuse.statistics.tracePassCount ==
                frequencyCount,
        "parallel and serial reuse trace once for "
        "1/2/16 frequencies");
    context.check(
        parallel.statistics.activeFrequencyLimit ==
    ...
    context.check(
        parallel.callbackCounts ==
            std::vector<std::size_t>(frequencyCount, 1U),
        "parallel callback consumes every frequency "
        "exactly once");
    context.check(
        parallel.statistics.frequencyTimings.size() ==
                frequencyCount &&
            parallel.statistics.rayCount ==
                serial.statistics.rayCount &&
...
void testRepeatedRunIsDeterministic(Context& context) {
  const SimulationCase simulation =
      makeSimulation(makeFrequencies(16U));
  const ParallelRayReuseSettings settings{
      .workerCount = 4U,
      .outputQueueCapacity = 1U,
      .memoryBudgetBytes = 0U};
  const StreamedParallelRun first =
      runParallel(simulation, settings);
  const StreamedParallelRun second =
      runParallel(simulation, settings);

  for (std::size_t index = 0U;
       index < first.workspaces.size(); ++index) {
    context.check(
        first.workspaces[index].has_value() &&
            second.workspaces[index].has_value(),
        "repeated parallel runs return every workspace");
    if (first.workspaces[index] &&
        second.workspaces[index]) {
      checkWorkspaceEqual(
          context, *first.workspaces[index],
          *second.workspaces[index],
...
void testMemoryBudget(Context& context) {
  const SimulationCase simulation =
      makeSimulation(makeFrequencies(16U));
  const StreamedParallelRun unrestricted =
      runParallel(
          simulation,
          ParallelRayReuseSettings{
              .workerCount = 4U,
  ...
  const std::size_t cacheBytes =
      unrestricted.statistics.rayCacheBytes;
  const std::size_t workspaceBytes =
      unrestricted.statistics.estimatedWorkspaceBytes;
  const std::size_t twoWorkerBudget =
      cacheBytes + 4U * workspaceBytes;

  const StreamedParallelRun constrained =
      runParallel(
          simulation,
          ParallelRayReuseSettings{
              .workerCount = 4U,
  ...
  context.check(
      constrained.statistics.activeFrequencyLimit == 2U &&
          constrained.statistics.estimatedPeakMemoryBytes <=
              twoWorkerBudget,
...
void testInvalidSettingsAndConsumerFailure(
    Context& context) {
  const SimulationCase simulation =
      makeSimulation(makeFrequencies(2U));
  context.expectThrows<ValidationError>(
      [&]() {
        static_cast<void>(
            ParallelRayReuseSolver::solveStreaming(
...
int main() {
  ...
  std::cout
      << "All Bellhop RayReuse parallel-ray-reuse-solver tests passed\n";
  return 0;
}
