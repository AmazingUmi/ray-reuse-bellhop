...
void testSeaSurfaceMirrorAndEvent(Context& context) {
  ...
  context.checkNear(
      kSoundSpeed * rayreuse::norm(result.reflectedState.slowness),
      1.0, 1.0e-14,
  ...
  context.checkNear(event.tangentSlowness,
                    rayreuse::dot(incident.slowness, {1.0, 0.0}),
                    1.0e-15, "event stores tangent component");
  context.checkNear(event.normalSlowness,
                    rayreuse::dot(incident.slowness, {0.0, -1.0}),
                    1.0e-15,
...
void testSeabedDynamicJump(Context& context) {
  ...
  const double csJump =
      -rayreuse::dot(gradient, reflectedUnit - incidentUnit);
  const double tangent =
      rayreuse::dot(incident.slowness, geometry.tangent);
  const double normal =
...
void testSurfaceSignAndCurvatureModes(Context& context) {
  ...
  const FlatBoundaryReflection standard =
      rayreuse::reflectAtFlatBoundary(
  ...
  const double topCnJump =
      rayreuse::dot(geometry.soundSpeedGradient,
