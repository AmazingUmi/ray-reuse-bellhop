...
BeamEpsilon pickMinimumWidthEpsilon(double frequency, double sourceSoundSpeed,
  ...
  const double halfWidth =
      std::sqrt(2.0 * sourceSoundSpeed * loopRangeMeters / angularFrequency);
  const double halfWidthSquared = halfWidth * halfWidth;
  if (!std::isfinite(angularFrequency) || !std::isfinite(halfWidth) ||
      halfWidth <= 0.0 || !std::isfinite(halfWidthSquared) ||
  ...
  const std::complex<double> optimal{
      0.0, (0.5 * angularFrequency) * halfWidthSquared};
  const std::complex<double> value = epsilonMultiplier * optimal;
  if (!std::isfinite(value.real()) || !std::isfinite(value.imag()) ||
      value.imag() <= 0.0) {
    throw ValidationError("minimum-width beam epsilon must be finite");
  }
