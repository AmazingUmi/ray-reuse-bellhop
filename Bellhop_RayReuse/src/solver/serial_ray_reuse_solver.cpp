...
SerialRayReuseStatistics SerialRayReuseSolver::solveStreaming(
    ...
    double loopRange,
    const RayReuseFrequencyConsumer& consumer,
  ...
  for (std::size_t frequencyIndex = 0U;
       frequencyIndex < frequencies.size();
       ++frequencyIndex) {
  ...
  if (verifyCacheFingerprint) {
    ...
    if (statistics.cacheFingerprintAfter !=
        statistics.cacheFingerprintBefore) {
      throw ValidationError(
          "serial ray-reuse modified the frozen ray cache");
    }
  }
...
SerialRayReuseResult SerialRayReuseSolver::solve(
  ...
  result.statistics = solveStreaming(
      ...
      [&result](std::size_t, FrequencyWorkspace&& workspace,
        ...
        result.frequencyResults.push_back(
            SerialRayReuseFrequencyResult{
