...
void storeLittleEndian(std::vector<std::byte>& record, std::size_t offset,
                       Unsigned value) {
  static_assert(std::is_unsigned_v<Unsigned>);
  if (offset > record.size() || sizeof(Unsigned) > record.size() - offset) {
    throw ValidationError("SHD record packing exceeded record size");
  }
  for (std::size_t byteIndex = 0U; byteIndex < sizeof(Unsigned); ++byteIndex) {
    record[offset + byteIndex] =
        static_cast<std::byte>(value & static_cast<Unsigned>(0xffU));
    value >>= 8U;
  }
}

void storeInt32(std::vector<std::byte>& record, std::size_t offset,
                std::int32_t value) {
  storeLittleEndian(record, offset, std::bit_cast<std::uint32_t>(value));
}

void storeFloat32(std::vector<std::byte>& record, std::size_t offset,
                  float value) {
  storeLittleEndian(record, offset, std::bit_cast<std::uint32_t>(value));
}

void storeFloat64(std::vector<std::byte>& record, std::size_t offset,
                  double value) {
  storeLittleEndian(record, offset, std::bit_cast<std::uint64_t>(value));
}

void storeText(std::vector<std::byte>& record, std::size_t offset,
               std::size_t width, std::string_view text) {
  if (offset > record.size() || width > record.size() - offset ||
      text.size() > width) {
    throw ValidationError("SHD text field exceeded record size");
  }
...
[[nodiscard]] std::int32_t checkedInt32(std::size_t value,
                                        std::string_view name) {
  if (value >
      static_cast<std::size_t>(std::numeric_limits<std::int32_t>::max())) {
    throw ValidationError(std::string(name) + " exceeds the SHD int32 limit");
  }
  return static_cast<std::int32_t>(value);
}

void writeRecord(std::ofstream& output, const std::vector<std::byte>& record) {
...
[[nodiscard]] float checkedFloat32(double value, std::string_view name) {
...
class ShdFrequencyWriter::Impl {
 public:
  Impl(const std::filesystem::path& path, std::string_view title,
    ...
    if (!output_.is_open()) {
      throw BellhopError("unable to open SHD output: " + path_.string());
    }
  ...
  void writeFrequency(std::size_t index, const FrequencyWorkspace& first,
    ...
    if (index >= frequencies_.size()) {
      throw ValidationError("SHD frequency index is out of range");
    }
    if (written_[index]) {
  ...
  void finalize() {
    ...
    output_.close();
    if (!output_) {
      throw BellhopError("failed to finalize SHD output: " + path_.string());
    }
    finalized_ = true;
  }
 ...
 private:
  ...
  void validateStaticData(const SimulationCase& simulation) const {
    if (frequencies_.empty()) {
      throw ValidationError("SHD output requires at least one frequency");
    }
    ...
    for (const double range : simulation.receivers().ranges()) {
  ...
  void calculateLayout() {
    ...
    const std::size_t recordWords = std::max(
        {kMinimumRecordWords, 2U * frequencies_.size(), 2U * bearingCount,
         2U * sourceXCount, 2U * sourceYCount, sourceDepthCount,
         receiverDepthCount_, 2U * receiverRangeCount_});
    if (recordWords > std::numeric_limits<std::size_t>::max() / 4U) {
    ...
    recordWords32_ = checkedInt32(recordWords, "SHD record word count");
    ...
    if (totalBytes_ >
        static_cast<std::size_t>(std::numeric_limits<std::streamoff>::max())) {
      throw ValidationError("SHD file exceeds stream offset capacity");
    }
  }

  void writeHeader(std::string_view title, const SimulationCase& simulation) {
    ...
    const std::int32_t receiverRangeCount32 =
        checkedInt32(receiverRangeCount_, "SHD receiver-range count");

    std::vector<std::byte> record(recordBytes_);
    const auto writeClearedRecord = [&] {
      writeRecord(output_, record);
      std::fill(record.begin(), record.end(), std::byte{});
    };
    ...
    for (std::size_t index = 0U; index < frequencies_.size(); ++index) {
      storeFloat64(record, 8U * index, frequencies_[index]);
    }
    writeClearedRecord();
    ...
    for (std::size_t index = 0U; index < receiverDepthCount_; ++index) {
      storeFloat32(
          record, 4U * index,
          checkedFloat32(receivers.depths()[index], "SHD receiver depth"));
    }
    writeClearedRecord();

    for (std::size_t index = 0U; index < receiverRangeCount_; ++index) {
      storeFloat64(record, 8U * index, receivers.ranges()[index]);
    }
    writeRecord(output_, record);
  }

  void preallocate() {
    output_.seekp(static_cast<std::streamoff>(totalBytes_ - 1U), std::ios::beg);
    output_.put('\0');
    output_.flush();
    if (!output_) {
      throw BellhopError("failed to preallocate SHD output: " + path_.string());
    }
  }

  void seekToRecord(std::size_t recordIndex) {
    const std::size_t byteOffset = recordIndex * recordBytes_;
    output_.seekp(static_cast<std::streamoff>(byteOffset), std::ios::beg);
    if (!output_) {
      throw BellhopError("failed to seek within SHD output: " + path_.string());
    }
  }
...
ShdFrequencyWriter::ShdFrequencyWriter(const std::filesystem::path& path,
                                       std::string_view title,
                                       const SimulationCase& simulation)
    : impl_(std::make_unique<Impl>(path, title, simulation)) {}

ShdFrequencyWriter::~ShdFrequencyWriter() noexcept = default;

ShdFrequencyWriter::ShdFrequencyWriter(ShdFrequencyWriter&&) noexcept = default;

ShdFrequencyWriter& ShdFrequencyWriter::operator=(
    ShdFrequencyWriter&&) noexcept = default;

void ShdFrequencyWriter::writeFrequency(std::size_t index,
...
void ShdFrequencyWriter::finalize() {
  if (!impl_) {
    throw ValidationError("cannot finalize a moved-from SHD writer");
  }
  impl_->finalize();
}

void ShdWriter::writeSingleFrequency(const std::filesystem::path& path,
                                     std::string_view title,
                                     const SimulationCase& simulation,
                                     const FrequencyWorkspace& workspace) {
  writeFrequencies(path, title, simulation,
