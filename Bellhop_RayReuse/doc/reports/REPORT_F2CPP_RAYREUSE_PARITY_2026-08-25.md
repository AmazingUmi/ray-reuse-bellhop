...
审计日期：2026-08-25；FP-2B 实现验证更新：2026-08-27
...
本报告在原始只读审计之后，按 FP-1A～FP-2B 的实际实现与验证结果增量更新。当前
声明范围严格限定为 `point + single source depth + rectilinear receiver + C-linear/PCHIP SSP`
...
source weighting。PCHIP `P` 已接入上述共享 geometry/frequency-local product 路径；
N²-linear `N`、spline `S` 和 Q/`.ssp` 仍明确不支持，其他原始 GAP 不因本次更新而放宽。
...
规则接收网格、C-linear 与 PCHIP `P` SSP、RR-B1 边界子集、单频 R，以及
Cartesian G/B 与 ray-centered GeoHat g 的 A/a/E；这些已可在 RayReuse 的
`nonreuse`、`reuse`、`parallel` 路径内按其适用范围使用。N/S/Q SSP 仍为
unsupported/deferred，不能从 PCHIP parity 外推为整个 SSP family 已闭环。
...
- Cartesian 与 ray-centered Cerveny 的
  ...
  family 在本报告的 point/single/rectilinear/C-or-P-SSP slice 内已闭环。
...
- geometry tracer/stepper 已通过最小 `GeometrySspEvaluator<CLinear,Pchip>` 解耦，
  frequency projection 使用独立的 `FrequencySspEvaluator<CLinear,Pchip>`；PCHIP
  的连续梯度与非零 `d²c/dz²` 进入 dynamic ray，N/S/Q backend 仍未实现且 parser 明确拒绝。
...
| `R-MODEL` | `Bellhop_RayReuse/include/rayreuse/model/simulation_case.hpp`、`environment.hpp`、`sound_speed_evaluator.hpp` 与 `src/model/simulation_case.cpp`：单 `Source`、Cartesian-product `ReceiverGrid`；`SoundSpeedProfile` 保存 C-linear/PCHIP kind，geometry 与 frequency-local evaluator 均为只含 C/P 的 value-owned variant；显式 C/I/S coherence、Cartesian/ray-centered Cerveny coordinate、`P/V/H` field component、`F/M/W` beam width、`D/S/Z` reflection curvature 与 complex-pressure/intensity workspace 选择；仍无 line/multisource/irregular 与 N/S/Q model backend |
...
| `R-TL` | `Bellhop_RayReuse/src/solver/single_frequency_solver.cpp`、`frequency_projector.cpp`、各 production Influence 与 `pressure_scaling.cpp`：source sampling、trace、Cartesian Cerveny local sampling 与逐频 projection 共用 C/P evaluator dispatch；C 使用 complex pressure，各 production-supported family 的 I/S 使用逐频 intensity；Cartesian/ray-centered Cerveny 共用每频每 ray F/M/W epsilon；ray-centered GeoHat 保持独立 traversal；G/B/S 使用 geometric point normalization；`.sbp` 与适用的 S Lloyd factor 在逐频 Project 前形成 source amplitude |
...
| `MATRIX` | `Bellhop_F2CPP/doc/reference/REFERENCE_FEATURE_SUPPORT_MATRIX.md` 与 `Bellhop_RayReuse/doc/reference/REFERENCE_FEATURE_SUPPORT_MATRIX.md`；RayReuse 矩阵已同步 FP-1A～FP-2B 实际支持面与剩余限制 |
| `STD` | `test/standard_cases/coverage.toml`、各 case `case.toml`、`codes/standard_cases.py`：共享 adapter/oracle 与版本 allow-list |
| `TEST` | FP-2B isolated clean RayReuse CTest 34/34；仓库全量 Python/pytest 168/168；standard-case unittest 153/153；`munk_pchip` single 与 broadband 三模式通过；既有 FP-2A full product matrix 继续由相关回归保护 |
...
| --- | --- | --- | --- | --- |
| TL-01 — coherence `C` 跨 F2CPP production TL beam/coordinate family | 全部正式 TL family 按其约束支持 | Cartesian/ray-centered Cerveny、Cartesian/ray-centered GeoHat、Cartesian GeoGaussian 与 Cartesian Simple Gaussian 在 point/single/rectilinear/C-or-P-SSP slice 内闭环 | `PARITY` | `F-PARSER`, `R-PARSER`, `F-TL`, `R-TL`, `TEST`, `FP2B-ORACLE`；line/multisource/irregular/N/S/Q SSP 分别由其他条目跟踪，不外推到那些维度 |
| TL-02 — Cartesian Cerveny，C/I/S、`{F,M,W}{D,S,Z}` beam options、P/V/H component、point/single/rectilinear/C-or-P-SSP | 支持 | 支持 | `PARITY` | `F-TL`, `R-TL`, `R-GEOM`, `STD`, `TEST`, `FP1A-ORACLE`, `FP1E-ORACLE`, `FP1F-ORACLE`, `FP1G-ORACLE`, `FP2B-ORACLE`；C 原路径保持，I/S 使用独立逐频 intensity workspace，P/V/H 保持 legacy identity |
...
| TL-06 — ray-centered Cerveny，C/I/S、`{F,M,W}{D,S,Z}`、P/V/H、point/single/rectilinear/C-or-P-SSP | 支持；要求至少两个等间距 receiver ranges | parser/model/runtime/Influence/SHD/oracle 闭环；保持 F2CPP projection/traversal、persistent image-normal flip、P/V/H derivative、逐 image I/S 与 receiver-level KMAH | `PARITY` | `F-PARSER`, `R-PARSER`, `F-TL`, `R-TL`, `R-MODEL`, `STD`, `TEST`, `FP1H-ORACLE`, `FP2B-ORACLE`；不外推到 irregular/N/S/Q SSP |
| TL-07 — Cartesian GeoHat，C/I/S、point/single/rectilinear/C-or-P-SSP | 支持 | parser/runtime/Influence/geometric scaling/SHD/oracle 闭环 | `PARITY` | `R-PARSER`, `R-TL`, `STD`, `TEST`, `FP1B-ORACLE`, `FP2B-ORACLE`；不外推到 line/multisource/irregular/N/S/Q SSP |
| TL-08 — ray-centered GeoHat C/I/S、point/single/rectilinear/C-or-P-SSP | 支持；至少两个等间距 receiver ranges | parser/model/runtime/Influence/geometric scaling/SHD/oracle 闭环；保持 F2CPP depth projection、range-index walker、same-index skip、right-endpoint acoustic state、linear hat 与 q-caustic order | `PARITY` | `F-PARSER`, `R-PARSER`, `F-TL`, `R-TL`, `R-MODEL`, `STD`, `TEST`, `FP1I-ORACLE`, `FP2B-ORACLE`；没有 Cerveny image loop/persistent flip/KMAH/window；相同 geometry primitive 的产品闭环由 PRD-05 跟踪 |
| TL-09 — Cartesian GeoGaussian，C/I/S、point/single/rectilinear/C-or-P-SSP | 支持 | parser/runtime/逐频 width 与 membership/Influence/geometric scaling/SHD/oracle 闭环 | `PARITY` | `R-PARSER`, `R-TL`, `STD`, `TEST`, `FP1C-ORACLE`, `FP2B-ORACLE`；`sigma_nf`、`sigma_lambda`、`sigma_1`、membership 与 Gaussian kernel 全部逐频计算 |
| TL-10 — ray-centered GeoGaussian | F2CPP 未正式支持 | 未支持 | `F2CPP_OUT_OF_SCOPE` | `MATRIX`, F2CPP parser/solver 均不提供该组合 |
| TL-11 — Cartesian Simple Gaussian coherent TL，point/single/rectilinear/C-or-P-SSP | 支持；不提供 I/S accumulator | parser/runtime/Influence/geometric scaling/SHD/oracle 闭环；`IS/SS` 明确拒绝 | `PARITY` | `R-PARSER`, `R-TL`, `STD`, `TEST`, `FP1D-ORACLE`, `FP2B-ORACLE`；保留 `0.98F` 混合精度、legacy SINT、严格 q crossing 与 right-endpoint acoustic state |
...
| SSP-03 — N2-linear | 支持 | Deferred/unsupported；parser 明确拒绝且无 N backend | `GAP` | `F-PARSER`, `F-GEOM`, `R-PARSER`, `R-MODEL` |
...
| PRD-05 — A/a/E ray-centered `g` | 支持；要求至少两个等间距 receiver ranges | parser/model/runtime/Arrival/Eigenray/writer/oracle 闭环；多频逐频独立发布，沿用既有命名与 cleanup lifecycle | `PARITY` | `F-PARSER`, `R-PARSER`, `R-MODEL`, `R-PRODUCT`, `R-CLI`, `STD`, `TEST`, `FP2A-ORACLE`, `FP2B-ORACLE`；不外推到 irregular/line/multisource/N/S/Q SSP |
...
| Feature | F2CPP | RayReuse | Status | Evidence |
| --- | --- | --- | --- | --- |
| DOC-01 — production support matrix 与真实 executable surface 同步 | 当前矩阵与 parser/solver/tests 基本一致 | FP-1A～FP-2A 已支持面，以及 FP-2B C-linear/PCHIP SSP、real geometry、frequency-local projection、TL/R/A/a/E 与 execution 范围已同步；N/S/Q 与其他限制保留 | `PARITY` | `MATRIX`、`TEST`、`FP2B-ORACLE` 与本报告逐项代码证据对照 |
...
| --- | --- | --- | --- |
| `nonreuse` | TL-02/03/04/05/06/07/09/11/12/14（含 {F,M,W}{D,S,Z}）；PRD-02/03/04/05；C-linear/PCHIP SSP；当前 BND/ATT parity 子集 | R 不接受显式 execution mode；SRC/REC、N/S/Q SSP 与其他 TL gap 保持不支持 | shared broadband 支持案例通过；PCHIP 每频独立 trace + solve/product |
...
5. `SSP-03`：N2-linear SSP（deferred/unsupported GAP）。
6. `SSP-04`：spline SSP（deferred/unsupported GAP）。
7. `SSP-05`：Q + `.ssp` range-dependent SSP（deferred/unsupported architectural conflict）。
8. `BND-04`：canonical curvilinear `C` boundary（architectural conflict）。
9. `BND-09`：flat `A` elastic P/S 缺 RayReuse executable oracle 闭环。
10. `ATT-01`：attenuation units N/F/M/Q/L 缺 RayReuse product-level oracle 闭环。
11. `ATT-04`：Francois–Garrison attenuation。
12. `ATT-05`：biological attenuation。
...
- **P0 — 主功能 / 后续架构**：SSP-03～SSP-05；SRC-04、REC-02、REC-03 所暴露的
...
FP-2B 已关闭 PCHIP `P` parity；本报告不指定下一 feature batch。N/S/Q、irregular
receiver、line/multisource、canonical curvilinear boundary、Influence Geometry Reuse
和频率插值仍为独立后续范围，不能从 FP-2B 的 C/P abstraction 推断已支持。
