...
> 封板日期：2026-08-20；FP-1A～FP-2I 最终封板：2026-08-29；最终分类措辞统一：2026-08-30；IGR-3A fused TL 与 IGR-3B fused Arrival 支持域封板：2026-09-04
...
> **Current support:** IGR-3A/IGR-3B 已 `ACCEPTED / CLOSED`。当前 fused
> production execution 覆盖下述 TL 与 Arrival 支持域，并可选用静态
> receiver-range parallelism。

## fused execution-mode 支持域

TL fused gate：TL run mode、该 run mode 对所选 beam family 合法、恰好 1 个
...
非 fused 限制）；fused Arrival 同样不支持 paired irregular receivers；`R/E`
产品不进入 fused。legacy `nonreuse/reuse/parallel` 行为不受影响。
...
| Arrivals | ASCII `A`、binary `a`；Cartesian geometric hat/Gaussian `G/B` 与 ray-centered GeoHat `g`；legacy frequency-local `ArrivalWorkspace` 与共享 AddArr 语义；fused 使用 source-local `[range][depth][frequency]` ordered variable-length lanes、static range ownership 与 zero-copy frequency view；点声源（`1/sqrt(range)`）与线声源（`4.0*sqrt(pi)`）幅值缩放；多 source depths 时 per-source 块与 header source depths（ASCII 行/binary count record，source depth 升序）；Cartesian `G/B` legacy 路径支持 paired irregular receivers，fused Arrival 仅支持规则网格 |
...
| 输出生命周期 | PRT、SHD、RAY、ARR；原子单文件发布；fused broadband ARR 先完成所有 per-frequency `.tmp` 再协调发布整组文件，失败时恢复已有 finals 并清理本次 partial finals/temp/backup；模式切换清理后不留下部分正式产品 |
...
| 并行 | worker 数运行时配置；legacy `parallel` 下 A/a/E 与 TL 支持外层 frequency parallelism，frequency worker 只读共享 per-source cache vector，consumer 按频率索引串行发布 per-source workspace 序列，不共享可变 writer；当前 fused TL 与 fused Arrival 支持显式 static contiguous range parallelism，各 worker 独占 range block，默认请求 4 workers |
...
header frequency 和文件名一一对应；`nonreuse/reuse/parallel` 以及上文支持域内
的 fused Arrival 每频产品逐字节一致。Origin 没有明确的多频 R 产品，因此多频
R 明确拒绝。

## IGR-3 closed architecture status

IGR-3 已将 Cross-Frequency Fused + Static Range Parallelism 封板为统一
Influence execution architecture。IGR-3A 把 TL beam families 适配进上文 fused
execution-mode 支持域；IGR-3B 把 `A/a` 的 `G/g/B` geometric contribution
...
直接消费 frequency view；AddArr 与 legacy 共享同一 semantic primitive。
...
| 并行所有权 | legacy `parallel` 选择 frequency worker；当前 fused TL/Arrival 可选 static range worker。两者都不把 frequency/source/receiver 任一层冻结为永久 owner，也不设置 4T/8T 程序级上限 |
