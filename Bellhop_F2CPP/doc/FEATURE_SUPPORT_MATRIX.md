...
复刻封板后只保留性能阶段入口，不在 B4 实施：

1. 对代表性 TL、R、A/E 工作负载做分阶段 profile，先确认真实热点；
2. 测量完整冻结轨迹的 peak RSS、分配次数和 cache locality，再决定数据布局优化；
3. 依据 profile 评估 receiver lookup、Influence 遍历和逐射线投影的算法复杂度，
   之后才考虑 SIMD/OpenMP 或批处理。
