...
// Matches the tracer/reflection guard; curved PCHIP profiles can accumulate
// O(1e-4) modified-box invariant drift without producing an invalid ray.
...
void validateFrozenMaterial(const FrozenBoundaryMaterial& frozen) {
  ...
  for (const RawAttenuation* attenuation :
    ...
    if (!validAttenuationUnit(attenuation->unit) ||
        ...
        attenuation->referenceFrequency <= 0.0 ||
...
void validateEvent(const ReflectionEvent& event,
  ...
  if (std::abs(tangentNorm - normalNorm) >
      ...
      std::abs(fortranDotProduct2D(event.boundaryTangent,
                                  event.outwardNormal)) >
  ...
  const double incidentTangent =
      fortranDotProduct2D(event.incidentSlowness,
                         event.boundaryTangent);
  ...
  const Vec2 expectedReflectedSlowness{
      .range = std::fma(-twiceIncidentNormal,
                        event.outwardNormal.range,
                        event.incidentSlowness.range),
      .depth = std::fma(-twiceIncidentNormal,
                        event.outwardNormal.depth,
  ...
  const bool unitFrame =
      std::abs(tangentNorm - 1.0) <= kGeometryTolerance &&
      std::abs(normalNorm - 1.0) <= kGeometryTolerance;
  if (unitFrame) {
    if (std::abs(incidentPoint.soundSpeed *
                     norm(event.incidentSlowness) -
                 1.0) > kSlownessNormTolerance ||
        std::abs(reflectedPoint.soundSpeed *
                     norm(event.reflectedSlowness) -
                 1.0) > kSlownessNormTolerance) {
      throw ValidationError(
