...
[[nodiscard]] double boundaryDistance(
  ...
  switch (boundary) {
    case ReflectionBoundary::SeaSurface:
      return seaSurfaceGeometry.interiorSignedDistance(
          position, boundarySegmentIndex);
    case ReflectionBoundary::Seabed:
      return seabedGeometry.interiorSignedDistance(
          position, boundarySegmentIndex);
  }
...
[[nodiscard]] std::optional<ReflectionBoundary> crossedBoundary(
    ...
    const BoundaryGeometry& seaSurfaceGeometry,
    const BoundaryGeometry& seabedGeometry,
    std::size_t initialTopSegment, std::size_t endTopSegment,
    std::size_t initialBottomSegment, std::size_t endBottomSegment) {
...
class GeometryStepLimiter {
 public:
  ...
  [[nodiscard]] double operator()(
      const StepLimitRequest& request, std::size_t topSegmentIndex,
    ...
    reduceForBoundary(
        request, trial, seaSurfaceGeometry_, topSegmentIndex, step);
    reduceForBoundary(
        request, trial, seabedGeometry_, bottomSegmentIndex, step);
    reduceForBoundarySegment(
        request, trial, topSegmentIndex, bottomSegmentIndex, step);
 ...
 private:
  static void reduceForSpatialCoordinate(double initialCoordinate,
    ...
    const double boxStep =
        (absoluteLimit - std::abs(initialCoordinate)) /
        std::abs(tangent);
    step = std::min(step, boxStep);
  }

  void reduceForDepthInterface(const StepLimitRequest& request,
                               Vec2 trial, double& step) const {
  ...
  static void reduceForBoundary(
      const StepLimitRequest& request, Vec2 trial,
      const BoundaryGeometry& geometry, std::size_t segmentIndex,
      double& step) {
    const BoundaryGeometrySample sample = geometry.evaluateAtSegment(
        request.initialPosition.range, segmentIndex);
    if (geometry.isFlat()) {
      const bool crosses =
          geometry.orientation() == BoundaryOrientation::Upper
              ? trial.depth < sample.point.depth
              : trial.depth > sample.point.depth;
      if (crosses) {
        reduceAtDepth(request.initialPosition.depth,
                      request.unitTangent.depth, sample.point.depth, step);
      }
      return;
    }

    const double trialExteriorDistance =
        fortranDotProduct2D(sample.outwardNormal,
                           trial - sample.point);
    if (trialExteriorDistance <= std::numeric_limits<double>::epsilon()) {
      return;
    }
    const double denominator =
        fortranDotProduct2D(request.unitTangent,
  ...
  void reduceForBoundarySegment(
      const StepLimitRequest& request, Vec2 trial,
      std::size_t topSegmentIndex, std::size_t bottomSegmentIndex,
      double& step) const {
    const BoundaryGeometrySample top =
        seaSurfaceGeometry_.evaluateAtSegment(
            request.initialPosition.range, topSegmentIndex);
    const BoundaryGeometrySample bottom =
        seabedGeometry_.evaluateAtSegment(
    ...
    if (trial.range < minimumRange) {
      reduceAtRange(request.initialPosition.range,
                    request.unitTangent.range, minimumRange, step);
    } else if (trial.range > maximumRange) {
      reduceAtRange(request.initialPosition.range,
                    request.unitTangent.range, maximumRange, step);
    }
  }
...
RayPath GeometryTracer::trace(const Source& source,
  ...
  const double sourceTopDistance =
      seaSurfaceGeometry.interiorSignedDistance(
          sourcePosition, topSegmentIndex);
  const double sourceBottomDistance =
      seabedGeometry.interiorSignedDistance(
          sourcePosition, bottomSegmentIndex);
  ...
  const SoundSpeedSample sourceSample =
      soundSpeedProfile_.evaluateAtSegments(
  ...
  const double waterDepth =
      seabedGeometry.referenceDepth() -
      seaSurfaceGeometry.referenceDepth();
  ...
  path.points.push_back(
      RayState{.position = sourcePosition,
               ...
               .dynamicP = {1.0, 0.0},
               .dynamicQ = rayTraceMode_ ? std::array<double, 2>{0.0, 0.0}
  ...
  const StepLimiter stepLimiter =
      [&geometryLimiter, &topSegmentIndex,
       &bottomSegmentIndex](const StepLimitRequest& request) {
        return geometryLimiter(
            request, topSegmentIndex, bottomSegmentIndex);
      };
  ...
  while (true) {
    ...
    try {
      ...
      path.terminationDetail = error.what();
      return path;
    }

    const std::size_t endTopSegment =
        seaSurfaceGeometry.locateSegment(
            result.endState.position.range, topSegmentIndex);
    const std::size_t endBottomSegment =
        seabedGeometry.locateSegment(
            result.endState.position.range, bottomSegmentIndex);
    const double beginTopDistance =
        boundaryDistance(path.points.back().position,
                         ReflectionBoundary::SeaSurface,
                         seaSurfaceGeometry, seabedGeometry,
                         topSegmentIndex);
    const double endTopDistance =
        boundaryDistance(result.endState.position,
                         ReflectionBoundary::SeaSurface,
                         seaSurfaceGeometry, seabedGeometry,
                         endTopSegment);
    const double beginBottomDistance =
        boundaryDistance(path.points.back().position,
    ...
    const std::optional<ReflectionBoundary> boundary =
        crossedBoundary(path.points.back().position,
                        result.endState.position, seaSurfaceGeometry,
                        seabedGeometry, topSegmentIndex, endTopSegment,
    ...
    if (boundary.has_value()) {
      ...
      try {
        ...
        const BoundaryGeometrySample collisionSample =
            boundaryGeometry.evaluateAtSegment(
                result.endState.position.range, boundarySegmentIndex);
        const BoundaryGeometrySample reflectionSample =
            boundaryGeometry.reflectionSampleAtSegment(
                result.endState.position, boundarySegmentIndex);
        reflection = reflectAtBoundary(
            result.endState, *boundary,
            BoundaryReflectionGeometry{
                .collisionPlanePoint = collisionSample.point,
                .collisionPlaneOutwardNormal =
                    collisionSample.outwardNormal,
                .reflectionTangent = reflectionSample.tangent,
                .reflectionOutwardNormal =
                    reflectionSample.outwardNormal,
                .soundSpeedGradient =
    ...
    if (beginTopDistance < 0.0 && endTopDistance < 0.0) {
      path.terminationReason = RayTerminationReason::ExitedDomain;
      path.terminationDetail = "two consecutive points outside sea surface";
      return path;
    }
    if (beginBottomDistance < 0.0 && endBottomDistance < 0.0) {
      path.terminationReason = RayTerminationReason::ExitedDomain;
      path.terminationDetail = "two consecutive points outside seabed";
      return path;
    }
    if (std::abs(path.points.back().dynamicQ[0]) > 1.0e100 ||
      ...
      path.terminationReason = RayTerminationReason::NumericalFailure;
      path.terminationDetail = "dynamic q exceeded the finite safety limit";
      return path;
    }
  }
}
