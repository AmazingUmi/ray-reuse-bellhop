...
void validateUniformReceiverRanges(const ReceiverGrid& receivers,
                                   double rangeDelta) {
  if (receivers.isIrregular()) {
    throw ValidationError(
        "ray-centered Cerveny does not yet support irregular receivers");
  }
...
void validateAccumulateInput(
  ...
  for (const RayFrequencyPoint& point : frequencyState.points) {
    ...
    requireFinite(
        point.reflectionPhase, "ray-centered Cerveny reflection phase");
...
RayCenteredCervenyInfluence::RayCenteredCervenyInfluence(
  ...
  if (settings_.beamWindow <= 0) {
    throw ValidationError(
        "ray-centered Cerveny beam window must be positive");
  }
  switch (widthMode_) {
  ...
  switch (sourceGeometry_) {
    ...
    case SourceGeometry::Line:
      break;
    default:
      throw ValidationError("ray-centered source geometry is invalid");
  }
...
RayCenteredCervenyInfluence::accumulateImpl(
  ...
  const PrecomputedRayValues ray = precomputeRayValues(
      path, epsilon, activePointCount, widthMode_);
  ...
  for (std::size_t depthIndex = 0U;
    ...
    for (std::size_t imageIndex = 0U;
      ...
      for (std::size_t rightIndex = 1U;
           rightIndex < activePointCount; ++rightIndex) {
        ...
        for (std::size_t receiverIndex1Based =
          ...
          if (fieldComponent_ != FieldComponent::Pressure) {
            ...
            if (fieldComponent_ == FieldComponent::Vertical) {
              ...
              // branch below is a handwritten expression and is not
              // conjugated in Origin.
              contribution =
                  soundSpeed *
          ...
          kmah = updateCervenyKmah(
              ray.q[rightIndex - 1U], q, kmah, widthMode_);
          if (kmah < 0) {
          ...
          requireFiniteComplex(
              contribution, "ray-centered pressure contribution");
          const double taper =
          ...
          if (pressureWorkspace != nullptr) {
            ...
            requireFiniteComplex(
                updated, "ray-centered accumulated pressure");
