...
[[nodiscard]] bool crossesSimpleGaussianCaustic(double previousQ,
...
void validateInput(
  ...
  if (workspace.frequency() != frequencyState.frequency ||
      workspace.depthCount() != receivers.receiversPerRange() ||
...
SimpleGaussianInfluence::SimpleGaussianInfluence(
    ...
    CervenyCoordinateSystem coordinates, SourceGeometry sourceGeometry,
  ...
  if (coordinates != CervenyCoordinateSystem::Cartesian) {
    throw ValidationError(
        "simple Gaussian influence requires Cartesian coordinates");
  }
  if (sourceGeometry != SourceGeometry::Point) {
    throw ValidationError(
        "simple Gaussian influence requires a point source");
  }
  if (runMode != SimulationRunMode::CoherentTransmissionLoss) {
    throw ValidationError(
        "simple Gaussian influence requires coherent transmission loss");
  }
  if (receivers_.isIrregular()) {
    throw ValidationError(
        "simple Gaussian influence requires a rectilinear receiver grid");
  }
}
...
std::optional<SimpleGaussianDiagnostic> SimpleGaussianInfluence::accumulate(
  ...
  for (std::size_t rightIndex = 1U; rightIndex < pointCount; ++rightIndex) {
    ...
    while (rangeIndex < receiverRanges.size() &&
      ...
      requireFinite(legacyArcLength,
                    "simple Gaussian legacy arc length");
      ...
      for (std::size_t depthIndex = 0U;
        ...
        requireFinite(offRayDistance,
                      "simple Gaussian off-ray distance");
        requireFinite(angularOffset,
                      "simple Gaussian angular offset");
