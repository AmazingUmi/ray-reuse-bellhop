#include "bellhop/numerics/pchip_coefficients.hpp"
...
double limitInterior(double firstSlope, double secondSlope,
                     double estimate) {
...
std::vector<std::complex<double>> clampedSplineDerivatives(
    ...
    std::complex<double> leftDerivative,
    std::complex<double> rightDerivative) {
...
std::vector<ComplexCubicPolynomial> computePchipCoefficients(
  ...
  std::vector<std::complex<double>> derivatives(nodeCount);
  {
    ...
    const std::complex<double> leftDerivative = limitParts(
        secants[0U], secants[1U], leftEstimate, limitLeftEndpoint);
    const std::complex<double> rightDerivative = limitParts(
        secants[last - 1U], secants[last], rightEstimate,
        limitRightEndpoint);
    derivatives = clampedSplineDerivatives(
        intervals, secants, leftDerivative, rightDerivative);
    derivatives.front() = leftDerivative;
    derivatives.back() = rightDerivative;
    for (std::size_t node = 1U; node + 1U < nodeCount; ++node) {
      derivatives[node] = limitParts(
          secants[node - 1U], secants[node], derivatives[node],
          limitInterior);
    }
  }
  ...
  for (std::size_t index = 0U; index + 1U < nodeCount; ++index) {
    ...
    const ComplexCubicPolynomial polynomial{
        ...
        .quadratic =
             (3.0 * difference -
              interval * (2.0 * derivatives[index] +
                          derivatives[index + 1U])) /
             (interval * interval),
        .cubic =
             (interval *
                  (derivatives[index] + derivatives[index + 1U]) -
              2.0 * difference) /
