...
std::size_t QuadrilateralSsp::locateRangeSegment(
  ...
  const double previousMinimum =
      grid_->rangesMeters[previousRangeSegment];
  const double previousMaximum =
      grid_->rangesMeters[previousRangeSegment + 1U];
  const bool isLast =
      previousRangeSegment + 1U == rangeSegmentCount();
...
SoundSpeedSample QuadrilateralSsp::evaluatePolynomial(
  ...
  const double cz1 =
      depthGradientAt(depthSegmentIndex, rangeSegmentIndex);
  const double cz2 =
