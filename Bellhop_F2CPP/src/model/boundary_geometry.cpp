...
BoundaryGeometry BoundaryGeometry::flat(
    double depth, BoundaryOrientation orientation) {
...
BoundaryGeometry::BoundaryGeometry(
  ...
  for (std::size_t index = 0U; index < nodes_.size(); ++index) {
    ...
    if (index > 0U &&
      ...
      throw ValidationError(
          "boundary node ranges must be strictly increasing");
    }
  }

  const auto makeSegment = [orientation](
                               double minimumRange,
    ...
    const double length =
        scale * std::sqrt(
                    scaledRange * scaledRange + scaledDepth * scaledDepth);
  ...
  segments_.push_back(makeSegment(
      -std::numeric_limits<double>::infinity(), nodes_.front().range,
      nodes_.front(), Vec2{.range = 1.0, .depth = 0.0}));
  for (std::size_t index = 0U; index + 1U < nodes_.size(); ++index) {
    segments_.push_back(makeSegment(
        nodes_[index].range, nodes_[index + 1U].range, nodes_[index],
        nodes_[index + 1U] - nodes_[index]));
  }
  segments_.push_back(makeSegment(
      nodes_.back().range, std::numeric_limits<double>::infinity(),
...
double BoundaryGeometry::depthAt(
  ...
  return sample.point.depth +
         (range - sample.point.range) * sample.tangent.depth /
             sample.tangent.range;
}

std::size_t BoundaryGeometry::locateSegment(
    double range, std::size_t previousSegment) const {
...
BoundaryGeometrySample BoundaryGeometry::evaluateAtSegment(
  ...
  if (isFlat()) {
    ...
    return {.point = Vec2{.range = 0.0, .depth = depth_},
            ...
            .outwardNormal =
                Vec2{.range = 0.0, .depth = upper ? -1.0 : 1.0},
            .curvature = 0.0,
...
double BoundaryGeometry::interiorSignedDistance(
  ...
  return -fortranDotProduct2D(sample.outwardNormal,
