...
void validateRawAttenuation(const RawAttenuation& attenuation,
  ...
  switch (attenuation.unit) {
    ...
    case AttenuationUnit::LossParameter:
      break;
    default:
      throw ValidationError(name + ".unit is invalid");
  }
}

void validateVolumeAttenuation(const VolumeAttenuation& attenuation) {
  const std::string name = "volumeAttenuation";
  switch (attenuation.model) {
    ...
    case VolumeAttenuationModel::Thorp:
      if (!std::holds_alternative<std::monostate>(
        ...
        throw ValidationError(
            name + ".parameters must be empty for None/Thorp");
      }
      break;
    case VolumeAttenuationModel::FrancoisGarrison: {
      if (!std::holds_alternative<FrancoisGarrisonParameters>(
              attenuation.parameters)) {
        throw ValidationError(
            name + " requires Francois-Garrison parameters");
      }
      const auto& parameters = std::get<FrancoisGarrisonParameters>(
          attenuation.parameters);
      requireFinite(parameters.temperatureCelsius,
                    name + ".temperatureCelsius");
      requireFinite(parameters.salinityPsu, name + ".salinityPsu");
      requireFinite(parameters.pH, name + ".pH");
      requireFinite(parameters.meanDepthMeters,
                    name + ".meanDepthMeters");
      if (parameters.temperatureCelsius <= -273.0 ||
          parameters.salinityPsu < 0.0 ||
          parameters.meanDepthMeters < 0.0) {
        throw ValidationError(
            name + " contains invalid Francois-Garrison parameters");
      }
      break;
    }
    case VolumeAttenuationModel::Biological: {
      if (!std::holds_alternative<SharedBiologicalAttenuationLayers>(
              attenuation.parameters)) {
        throw ValidationError(name + " requires biological layers");
      }
      const auto& sharedLayers =
          std::get<SharedBiologicalAttenuationLayers>(
          attenuation.parameters);
      if (!sharedLayers) {
        throw ValidationError(name + " requires non-null biological layers");
      }
      const auto& layers = *sharedLayers;
      if (layers.size() > 200U) {
        throw ValidationError(name + " exceeds 200 biological layers");
      }
      for (const auto& layer : layers) {
        requireFinite(layer.minimumDepth, name + ".layer.minimumDepth");
        requireFinite(layer.maximumDepth, name + ".layer.maximumDepth");
        requireFinite(layer.resonanceFrequency,
                      name + ".layer.resonanceFrequency");
        requireFinite(layer.qualityFactor,
                      name + ".layer.qualityFactor");
        requireFinite(
            layer.attenuationCoefficientDecibelsPerKilometer,
            name + ".layer.attenuationCoefficientDecibelsPerKilometer");
        if (layer.minimumDepth > layer.maximumDepth ||
          ...
          throw ValidationError(
              name + " contains an invalid biological layer");
        }
      }
      break;
    }
    default:
      throw ValidationError(name + " has an unknown model");
  }
}
...
[[nodiscard]] GrainSizeMaterial grainSizeToGeoacoustic(
    double meanGrainSize) {
  requireFinite(meanGrainSize, "grainSize.meanGrainSize");

  // AttenMod.f90 spells every decimal below as default REAL.  Promote the
  // binary32 constants explicitly, including the non-exact branch limits,
  // and spell out gfortran -O2's multiply/add contraction order.
  const auto legacy = [](float value) {
  ...
  if (meanGrainSize >= -1.0 && meanGrainSize < 1.0) {
    soundSpeedRatio =
        std::fma(-legacy(0.056452F), meanGrainSize,
                 legacy(0.002709F) * squared) +
        legacy(1.2778F);
    densityRatio =
        std::fma(-legacy(0.17057F), meanGrainSize,
                 legacy(0.007797F) * squared) +
        legacy(2.3139F);
  } else if (meanGrainSize >= 1.0 &&
             meanGrainSize < legacy(5.3F)) {
    const double cubed = squared * meanGrainSize;
    const double soundSpeedLeading =
        std::fma(legacy(0.0213937F), squared,
                 -legacy(0.0014881F) * cubed);
    soundSpeedRatio =
        std::fma(-legacy(0.1382798F), meanGrainSize,
                 soundSpeedLeading) +
        legacy(1.3425F);
    const double densityLeading =
        std::fma(legacy(0.2290201F), squared,
                 -legacy(0.0165406F) * cubed);
    densityRatio =
        std::fma(-legacy(1.1069031F), meanGrainSize, densityLeading) +
        legacy(3.0455F);
  } else {
    soundSpeedRatio = std::fma(-legacy(0.0024324F), meanGrainSize,
                               legacy(1.0019F));
    densityRatio = std::fma(-legacy(0.0012973F), meanGrainSize,
                            legacy(1.1565F));
  }
  ...
  } else if (meanGrainSize >= 0.0 &&
             meanGrainSize < legacy(2.6F)) {
    attenuationCoefficient =
        std::fma(legacy(0.0245F), meanGrainSize, legacy(0.4556F));
  } else if (meanGrainSize >= legacy(2.6F) &&
             meanGrainSize < legacy(4.5F)) {
    attenuationCoefficient =
        std::fma(legacy(0.1245F), meanGrainSize, legacy(0.1978F));
  } else if (meanGrainSize >= legacy(4.5F) &&
             meanGrainSize < legacy(6.0F)) {
    attenuationCoefficient =
        std::fma(legacy(0.20098F), squared,
                 std::fma(-legacy(2.5228F), meanGrainSize,
                          legacy(8.0399F)));
  } else if (meanGrainSize >= legacy(6.0F) &&
             meanGrainSize < legacy(9.5F)) {
    attenuationCoefficient =
        std::fma(legacy(0.0117F), squared,
                 std::fma(-legacy(0.2041F), meanGrainSize,
                          legacy(0.9431F)));
  } else {
  ...
  if (!std::isfinite(soundSpeedRatio) || soundSpeedRatio <= 0.0 ||
      ...
      !std::isfinite(attenuationCoefficient) ||
...
void validateQuadrilateralGrid(const SharedQuadrilateralSspGrid& grid,
  ...
  for (std::size_t index = 0U; index < grid->rangeCount; ++index) {
    ...
    if (index > 0U &&
      ...
      throw ValidationError("quadrilateral SSP ranges must be strictly increasing");
    }
  }
...
BoundaryModel::BoundaryModel(BoundaryKind kind, BoundaryGeometry geometry,
  ...
  if (kind_ != BoundaryKind::AcousticHalfSpace &&
      longMaterials_) {
    throw ValidationError(
...
Environment::Environment(SoundSpeedProfile soundSpeedProfile,
  ...
  if (soundSpeedProfile_.minimumDepth() >
      ...
      soundSpeedProfile_.maximumDepth() <
          seabed_.geometry().maximumDepth()) {
    throw ValidationError(
