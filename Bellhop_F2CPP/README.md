# Bellhop F2CPP

`Bellhop_F2CPP` 是可独立配置、构建和运行的 C++20 单频二维 Bellhop
实现。它支持目标范围内的 Cartesian/ray-centered Cerveny coherent、
incoherent 与 semi-coherent 二维声场和二维几何射线输出，从 Bellhop
`.env` 读取环境；TL 模式输出 PRT/SHD，ray-trace 与 eigenray 模式输出
PRT/RAY，arrival 模式输出 PRT/ARR。

核心数据流：

```text
.env
  → SimulationCase
  → GeometryTracer
  → frozen RayPathCache
  ├─ C/I/S TL  → FrequencyProjector → Influence → pressure/intensity → .shd
  ├─ R         → FrequencyProjector → RayWriter → .ray
  ├─ A/a       → ArrivalSolver → ArrivalWriter → .arr
  └─ E         → EigenraySolver → EigenrayWriter → .ray
```

程序保留完整、频率无关、冻结只读的射线轨迹缓存；复走时、衰减、反射
幅相和压力只存在于逐频临时状态。F2CPP 一次运行只计算一个频率，实际多频
调度和轨迹复用由独立的 `Bellhop_RayReuse` 工程实现。

## 快速开始

从本目录编译并测试 Release：

```bash
uv run cmake --preset release
uv run cmake --build --preset release --parallel
uv run ctest --preset release
```

可执行文件：

```text
build/release/bellhop_f2cpp
```

运行：

```bash
./build/release/bellhop_f2cpp <file-root>
```

参数必须省略 `.env` 扩展名。程序读取 `<file-root>.env`，并在相同位置
写出 `<file-root>.prt`；根据运行类型另写 `<file-root>.shd`、`.ray` 或 `.arr`。

## 文档

- [完整使用说明](./doc/guides/GUIDE_USAGE.md)：环境、编译、测试、CLI、输入输出和排错；
- [当前进度](./doc/status/STATUS_PROGRESS.md)：已完成范围、最新验证基线和当前施工入口；
- [二维单频支持矩阵](./doc/reference/REFERENCE_FEATURE_SUPPORT_MATRIX.md)：supported、intentional
  divergence 和 deferred/out-of-scope 的封板定义；
- [F2CPP 文档索引](./doc/README.md)；
- [历史构建与验收计划](./doc/archive/PLAN_BUILD_ACCEPTANCE.md)；
- [历史二维功能复刻计划](./doc/archive/PLAN_FEATURE_REPLICATION.md)；
- [最终派生清单](./doc/archive/REPORT_DERIVATION_MANIFEST_2026-07-29.md)；
- [数值接口与中间状态契约 v1](./doc/reference/REFERENCE_INTERMEDIATE_STATE_CONTRACT.md)。

全项目设计与数值契约：

- [Bellhop 源码分析与宽带复用设计](../doc/architecture/ARCHITECTURE_BELLHOP_RAY_REUSE.md)；
- [当前工作与待决事项](../doc/plans/PLAN_CURRENT_WORK.md)；
- [基础变量、单位与数值规范](../doc/reference/REFERENCE_NUMERICAL_CONVENTIONS.md)。

## 当前状态

I0～I8 与 I9-B1～B3 已全部验收并冻结，I9-B4 已完成二维单频复刻封板。
封板快照为 AppleClang/GCC 14 CTest 37/37、当时的 Python 145/145 和
F2CPP 单频标准案例 65/65；这些是 2026-08-16 的验收记录，不是动态的当前
测试总数。详细数值证据见[封板状态](./doc/status/STATUS_PROGRESS.md)，当前 Python
回归从仓库根目录运行 `uv run pytest`。

当前可正式声明的范围、兼容语义及明确延期项以
[二维单频支持矩阵](./doc/reference/REFERENCE_FEATURE_SUPPORT_MATRIX.md)为准。P1～P4-02 性能阶段
已完成并暂停；单 source Cartesian Cerveny TL 可通过 `F2CPP_THREADS` 使用
保持 SHD bitwise 一致的确定性 receiver-depth team，详见
[性能阶段记录](./doc/reports/REPORT_PERFORMANCE_2026-08-16.md)与[完整使用说明](./doc/guides/GUIDE_USAGE.md)。除非
发现真实 correctness bug，不再重新打开已冻结 iteration，也不在 F2CPP 中
顺带实现 3D、N×2D、beam shift 或宽带调度。
