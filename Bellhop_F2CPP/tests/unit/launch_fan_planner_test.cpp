...
using bellhop::LaunchFanPlan;
using bellhop::LaunchFanPlanner;
using bellhop::LaunchFanPlanningInput;
...
void testExplicitCountIsIgnored(Context& context) {
  ...
  input.explicitLaunchAngleCount =
...
void testFortranDegreeSubtabulationBits(Context& context) {
  LaunchFanPlanningInput input{
      ...
      .inputDegreeBounds =
          bellhop::LaunchAngleDegreeBounds{
              .minimum = -89.0,
              .maximum = 89.0},
  };
  const LaunchFanPlan plan = LaunchFanPlanner::plan(input);
  ...
  const std::vector<ExpectedBits> expected{
      {0U, 0xbff8da7e39bae2a3ULL},
      {1U, 0xbff8d2620039429dULL},
      {392U, 0xbfe8de8c567bb2a6ULL},
      {395U, 0xbfe8ade2fd71f281ULL},
      {784U, 0xbf50387303400ad2ULL},
      {785U, 0x3f50387303400d83ULL},
      {1568U, 0x3ff8d2620039429eULL},
      {1569U, 0x3ff8da7e39bae2a4ULL},
  };
  for (const ExpectedBits item : expected) {
    const std::uint64_t actual =
        std::bit_cast<std::uint64_t>(
            plan.launchAngles[item.index]);
    context.check(
        actual == item.bits,
        "launch angle index " + std::to_string(item.index) +
            " preserves Fortran degree-subtabulation bits; actual=" +
            std::to_string(actual) + ", expected=" +
            std::to_string(item.bits));
  }
  context.check(
      std::bit_cast<std::uint64_t>(plan.launchAngleStep) ==
...
void testInvalidInputs(Context& context) {
  const auto expectInvalid =
      [&context](LaunchFanPlanningInput input, const char* message) {
        context.expectThrows<ValidationError>(
            [&input] {
              static_cast<void>(LaunchFanPlanner::plan(input));
            },
            message);
      };
  ...
  input.inputDegreeBounds =
      bellhop::LaunchAngleDegreeBounds{
          ...
          .maximum = 5.0};
  expectInvalid(
      input, "a non-finite parser degree bound is rejected");
  ...
  expectInvalid(
      input, "a reversed parser degree interval is rejected");

  input = directCaseInput();
  input.inputDegreeBounds =
      bellhop::LaunchAngleDegreeBounds{
          .minimum = -6.0, .maximum = 5.0};
  expectInvalid(
      input, "degree provenance inconsistent with radians is rejected");
  ...
  input.frequencies = {
      static_cast<double>(std::vector<double>{}.max_size()) * 10000.0};
...
int main() {
  ...
  std::cout << "All Bellhop F2CPP launch fan planner tests passed\n";
  return 0;
}
