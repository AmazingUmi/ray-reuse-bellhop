...
using bellhop::RayCenteredCervenyDiagnostic;
using bellhop::RayCenteredCervenyDiagnosticRequest;
using bellhop::RayCenteredCervenyInfluence;
...
RayPath makeSlantedPath(std::size_t pointCount, double dynamicP) {
  const std::vector<bellhop::Vec2> positions{
...
RayCenteredCervenyDiagnostic runComponentDiagnostic(
  ...
  const RayCenteredCervenyInfluence influence(
      ...
      SimulationRunMode::CoherentTransmissionLoss, component);
...
void testPressureAndVelocityComponents(Context& context) {
  ...
  checkComplexNear(context, pressure.pressureContribution,
                   expectedPressure, 1.0e-13,
                   "pressure diagnostic contribution");
  ...
  const std::complex<double> alongDerivative =
      ...
      pressure.pressureContribution;
  const double slownessRange = 0.8 / kSoundSpeed;
  const double slownessDepth = 0.6 / kSoundSpeed;
  const std::complex<double> expectedVertical =
      kSoundSpeed *
      (std::conj(normalDerivative) * slownessRange +
       std::conj(alongDerivative) * slownessDepth);
  const std::complex<double> expectedHorizontal =
      kSoundSpeed *
      (-normalDerivative * slownessDepth +
       alongDerivative * slownessRange);
  checkComplexNear(context, vertical.pressureContribution,
                   expectedVertical, 1.0e-13,
                   "V conjugates both Pn and Ps like Fortran DOT_PRODUCT");
  ...
  const std::complex<double> nonConjugatedVertical =
      kSoundSpeed *
      (normalDerivative * slownessRange +
       alongDerivative * slownessDepth);
  context.check(
      std::abs(vertical.pressureContribution - nonConjugatedVertical) >
          1.0e-3,
      "V oracle is sensitive to the legacy complex conjugation");
}
...
void testLegacyImageNormalFlipsAndPerImagePower(Context& context) {
  ...
  checkComplexNear(context, threeImages - twoImages, base, 1.0e-13,
                   "bottom inherits and continues the vector-wide flips");

  const double basePower = std::abs(base) * std::abs(base);
  const double incoherentOne = runIntensityImages(
      1U, SimulationRunMode::IncoherentTransmissionLoss);
  const double incoherentTwo = runIntensityImages(
      2U, SimulationRunMode::IncoherentTransmissionLoss);
  const double semiCoherentTwo = runIntensityImages(
      2U, SimulationRunMode::SemiCoherentTransmissionLoss);
  ...
  context.checkNear(std::abs(twoImages) * std::abs(twoImages), 0.0,
                    1.0e-24,
...
void testNearHorizontalStepDoesNotFlipImageNormals(Context& context) {
  ...
  checkComplexNear(
      context, withSurface - trueImage, {}, 1.0e-13,
...
void testWkbReceiverInterpolationUpdatesKmah(Context& context) {
  ...
  checkComplexNear(context, before.qInterpolated, {0.5, 0.0}, 0.0,
                   "WKB receiver q before the real zero");
  checkComplexNear(context, after.qInterpolated, {-0.5, 0.0}, 0.0,
                   "WKB receiver q after the real zero");
  ...
  context.expectThrows<ValidationError>(
      [&] {
        FrequencyWorkspace workspace(1.0, receivers);
        const RayCenteredCervenyInfluence wkb(
            environment, receivers, {}, BeamWidthMode::Wkb);
        static_cast<void>(wkb.accumulate(
            workspace, path, frequencyState, {0.0, 1.0}));
      },
...
void testHermiteAppliedOnceToIntensity(Context& context) {
  ...
  context.check(
      std::abs(expected -
               expectedTaper * expectedTaper * magnitude * magnitude) > 1.0,
      "Hermite fixture distinguishes H from H squared");
}
  ...
  context.expectThrows<ValidationError>(
      [&] {
        static_cast<void>(RayCenteredCervenyInfluence(
            environment,
            ReceiverGrid({100.0, 200.0}, {100.0, 200.0},
                         ReceiverGridLayout::Irregular)));
      },
      "ray-centered influence rejects irregular receiver grids");
}
