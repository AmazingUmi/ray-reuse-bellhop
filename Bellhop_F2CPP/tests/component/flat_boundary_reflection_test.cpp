...
RayState makeState(double angleRadians, Vec2 position) {
  return RayState{
      .position = position,
      .slowness =
          {std::cos(angleRadians) / kSoundSpeed,
...
void testSeaSurfaceMirrorAndEvent(Context& context) {
  ...
  const FlatBoundaryReflection result = bellhop::reflectAtFlatBoundary(
  ...
  checkVectorNear(context, result.reflectedState.position,
                  incident.position, 0.0,
                  "D-10 reflection keeps the same position");
  ...
  context.checkNear(
      kSoundSpeed * bellhop::norm(result.reflectedState.slowness),
      1.0, 1.0e-14,
      "surface reflection preserves the sound-speed slowness norm");
  ...
  checkVectorNear(context, event.incidentSlowness, incident.slowness,
                  0.0, "event stores incident slowness");
  ...
  context.checkNear(event.tangentSlowness,
                    bellhop::dot(incident.slowness, {1.0, 0.0}),
                    1.0e-15, "event stores tangent component");
  context.checkNear(event.normalSlowness,
                    bellhop::dot(incident.slowness, {0.0, -1.0}),
                    1.0e-15,
...
void testSeabedDynamicJump(Context& context) {
  ...
  const Vec2 reflectedUnit =
      kSoundSpeed * result.reflectedState.slowness;
  const Vec2 incidentRayNormal{-incidentUnit.depth,
                               incidentUnit.range};
  const Vec2 reflectedRayNormal{reflectedUnit.depth,
                                -reflectedUnit.range};
  const double cnJump =
      -bellhop::dot(gradient, reflectedRayNormal - incidentRayNormal);
  const double csJump =
      -bellhop::dot(gradient, reflectedUnit - incidentUnit);
  const double tangent =
      bellhop::dot(incident.slowness, geometry.tangent);
  const double normal =
      bellhop::dot(incident.slowness, geometry.outwardNormal);
  const double ratio = tangent / normal;
  const double expectedJump =
      ratio * (2.0 * cnJump - ratio * csJump) /
      (kSoundSpeed * kSoundSpeed);

  for (std::size_t index = 0; index < incident.dynamicP.size();
       ++index) {
    context.checkNear(
        result.reflectedState.dynamicP[index],
        incident.dynamicP[index] +
            incident.dynamicQ[index] * expectedJump,
        1.0e-14,
...
void testSurfaceSignAndCurvatureModes(Context& context) {
  ...
  const Vec2 incidentUnit = kSoundSpeed * incident.slowness;
  const Vec2 reflectedUnit =
      kSoundSpeed * standard.reflectedState.slowness;
  const Vec2 incidentRayNormal{-incidentUnit.depth,
                               incidentUnit.range};
  const Vec2 reflectedRayNormal{reflectedUnit.depth,
                                -reflectedUnit.range};
  const double topCnJump =
      bellhop::dot(geometry.soundSpeedGradient,
...
void testInvalidGeometry(Context& context) {
  ...
  context.expectThrows<ValidationError>(
      [&] {
        (void)bellhop::reflectAtFlatBoundary(
  ...
  context.expectThrows<ValidationError>(
      [&] {
        (void)bellhop::reflectAtFlatBoundary(
            surfaceIncident, ReflectionBoundary::SeaSurface,
            nonOrthogonal, 0U,
            BoundaryCurvatureMode::Standard);
      },
  ...
  nonFinite.point.range =
      std::numeric_limits<double>::quiet_NaN();
  context.expectThrows<ValidationError>(
      [&] {
        (void)bellhop::reflectAtFlatBoundary(
            surfaceIncident, ReflectionBoundary::SeaSurface, nonFinite,
            0U, BoundaryCurvatureMode::Standard);
      },
      "non-finite boundary geometry is rejected");

  const RayState offBoundary =
      makeState(-kPi / 6.0, {25.0, 1.0});
  context.expectThrows<ValidationError>(
      [&] {
        (void)bellhop::reflectAtFlatBoundary(
            offBoundary, ReflectionBoundary::SeaSurface, makeSurface(),
            0U, BoundaryCurvatureMode::Standard);
      },
      "incident point away from the boundary is rejected");

  const RayState minimumStepOvershoot =
      makeState(-kPi / 6.0, {25.0, -0.0025});
  FlatBoundaryGeometry overshootGeometry = makeSurface();
  overshootGeometry.maximumIncidentPlaneDistance = 0.01;
  const FlatBoundaryReflection overshoot =
      bellhop::reflectAtFlatBoundary(
          minimumStepOvershoot, ReflectionBoundary::SeaSurface,
          overshootGeometry, 1U, BoundaryCurvatureMode::Standard);
  context.check(
      overshoot.reflectedState.position ==
          minimumStepOvershoot.position,
      "accepted minimum-step overshoot is reflected without projection");

  RayState accumulatedDrift =
      makeState(-kPi / 6.0, {25.0, 0.0});
  accumulatedDrift.slowness =
      1.00005 * accumulatedDrift.slowness;
  const FlatBoundaryReflection drifted =
      bellhop::reflectAtFlatBoundary(
          accumulatedDrift, ReflectionBoundary::SeaSurface,
          makeSurface(), 1U, BoundaryCurvatureMode::Standard);
  context.check(
      drifted.reflectedState.slowness.range ==
          accumulatedDrift.slowness.range,
      "reflection accepts bounded modified-box slowness drift");

  RayState corruptSlowness =
      makeState(-kPi / 6.0, {25.0, 0.0});
  corruptSlowness.slowness = 1.001 * corruptSlowness.slowness;
  context.expectThrows<ValidationError>(
      [&] {
        (void)bellhop::reflectAtFlatBoundary(
            corruptSlowness, ReflectionBoundary::SeaSurface,
            makeSurface(), 1U, BoundaryCurvatureMode::Standard);
      },
