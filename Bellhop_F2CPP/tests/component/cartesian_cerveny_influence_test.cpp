...
using bellhop::CartesianCervenySettings;
using bellhop::CervenyImageKind;
...
using bellhop::updateCervenyKmah;

constexpr double kDirectLaunchAngle =
...
void checkFloatBits(Context& context, std::complex<double> value,
                    std::uint32_t expectedReal,
                    std::uint32_t expectedImaginary,
                    const std::string& message) {
  const std::complex<float> quantized{
      static_cast<float>(value.real()),
      static_cast<float>(value.imag())};
  context.check(
      std::bit_cast<std::uint32_t>(quantized.real()) ==
          expectedReal,
      message + " real float bits");
  context.check(
      std::bit_cast<std::uint32_t>(quantized.imag()) ==
          expectedImaginary,
      message + " imaginary float bits");
}

void checkImage(Context& context,
                const bellhop::CartesianCervenyImageDiagnostic& image,
                CervenyImageKind kind, double deltaDepth,
                double windowMetric, bool passed, double taper,
  ...
  checkComplexNear(context, image.contribution, contribution,
                   1.0e-11, message + " contribution");
}
...
void testDirectOracle(Context& context) {
  ...
  const CartesianCervenyDiagnostic result =
      runDirectOracle(context, workspace);

  context.check(result.evaluationCount == 1U,
                "direct receiver is owned by one ray segment");
  context.check(result.leftPointIndex == 108U &&
                    result.rightPointIndex == 109U,
                "direct Fortran point indices map to C++ zero base");
  context.checkNear(result.interpolationWeight,
                    4.59987619743631741e-6, 1.0e-14,
                    "direct interpolation weight");
  context.checkNear(result.interpolatedPosition.range, 1080.0,
                    1.0e-10, "direct interpolated range");
  context.checkNear(result.interpolatedPosition.depth,
                    4.99684790025809491e2, 1.0e-8,
                    "direct interpolated depth");
  checkComplexNear(
      context, result.qInterpolated,
      {1.62000006899814308e6, 3.75000000000000047e6},
      1.0e-7, "direct interpolated q");
  checkComplexNear(
      context, result.gammaInterpolated,
      {4.85410684147624472e-8, -1.12363579521666120e-7},
      1.0e-15, "direct interpolated gamma");
  context.check(result.kmahLeft == 1 && result.kmahFinal == 1,
                "direct KMAH stays positive");
  checkComplexNear(
      context, result.constantCorrected,
      {3.10087839855024470e1, -2.03827780553720643e1},
      2.0e-12, "direct corrected constant");
  checkImage(
      context, result.images[0U], CervenyImageKind::True,
      3.15209974190509001e-1, 3.50731959363283536e-6,
      true, 1.0,
      {9.99996492653599689e-1, 8.11876686914479274e-6},
      "direct true image");
  checkImage(
      context, result.images[1U], CervenyImageKind::Surface,
      -9.99684790025809434e2, 3.52778092412033430e1,
      false, 9.65913706549500928e-1, {},
      "direct surface image");
  checkImage(
      context, result.images[2U], CervenyImageKind::Bottom,
      1.00031520997419057e3, 3.53223169646996027e1,
      false, 9.65498615397307192e-1, {},
      "direct bottom image");
  checkComplexNear(
      context, result.finalContribution,
      {3.10088407099787347e1, -2.03824548128207503e1},
      4.0e-11, "direct final ray contribution");
  checkComplexNear(
      context, workspace.at(10U, 10U),
      result.finalContribution, 0.0,
      "direct double workspace receives unquantized contribution");
  checkFloatBits(context, result.finalContribution, 0x41f8121bU,
                 0xc1a30f44U, "direct legacy quantization");
}
...
MunkOracleRun runMunkOracles(Context& context) {
  ...
  const ReceiverGrid receivers(
      linearGrid(0.0, 5000.0, 201U),
      linearGrid(0.0, 100000.0, 501U));
  const RayPath path =
      GeometryTracer(
          environment, bellhop::test::makeMunkIntegratorSettings())
          .trace(Source{.depth = 1000.0},
                 kMunkInfluenceLaunchAngle);
  const RayFrequencyState frequencyState =
      FrequencyProjector(environment).project(path, 50.0, 1.0);
  const auto epsilon = pickMinimumWidthEpsilon(
...
void testMunkOracles(Context& context) {
  ...
  context.check(focus.leftPointIndex == 53U &&
                    focus.rightPointIndex == 54U,
                "Munk focus point indices map to C++ zero base");
  context.checkNear(focus.interpolationWeight,
                    1.72167933213289992e-1, 1.0e-14,
                    "Munk focus interpolation weight");
  checkComplexNear(
      context, focus.qInterpolated,
      {-6.05129208201102010e4, -3.78017538757674024e7},
      1.0e-6, "Munk focus interpolated q");
  checkComplexNear(
      context, focus.gammaInterpolated,
      {7.30334138896506214e-9, -1.27190553964798013e-8},
      2.0e-15, "Munk focus interpolated gamma");
  context.check(focus.kmahLeft == 1 && focus.kmahFinal == 1,
                "Munk real-q caustic does not falsely flip KMAH");
  checkComplexNear(
      context, focus.constantCorrected,
      {2.70620424359742024e1, 2.71053979366365496e1},
      4.0e-10, "Munk focus corrected constant");
  checkImage(
      context, focus.images[0U], CervenyImageKind::True,
      9.70946017883443346, 3.76699376501307215e-4,
      true, 1.0,
      {-6.70864582211274985e-1, -7.41071924522222147e-1},
      "Munk focus true image");
  context.check(!focus.images[1U].windowPassed &&
                    !focus.images[2U].windowPassed,
                "Munk focus rejects both image beams");
  checkComplexNear(
      context, focus.finalContribution,
      {1.93208362125029010, -3.82389713319614799e1},
      8.0e-9, "Munk focus final ray contribution");
  checkFloatBits(context, focus.finalContribution, 0x3ff74e84U,
                 0xc218f4b5U, "Munk focus legacy quantization");

  const CartesianCervenyDiagnostic& branch = run.branch;
  context.check(branch.leftPointIndex == 186U &&
                    branch.rightPointIndex == 187U,
                "Munk negative-KMAH indices map to C++ zero base");
  context.check(branch.kmahLeft == -1 &&
                    branch.kmahFinal == -1,
                "Munk prior branch crossing remains negative");
  checkComplexNear(
      context, branch.constantPrincipal,
      {4.78825735552960374e-1, -5.03903428481553988e1},
      8.0e-10, "Munk principal constant");
  checkComplexNear(
      context, branch.constantCorrected,
      {-4.78825735552960374e-1, 5.03903428481553988e1},
      8.0e-10, "negative KMAH reverses the complex root");
  checkImage(
      context, branch.images[0U], CervenyImageKind::True,
      -2.63179390808556946, 8.39963404413040946e-5,
      true, 1.0,
      {-2.80901139404037714e-1, -9.59649191793861323e-1},
      "Munk branch true image");
  checkImage(
      context, branch.images[1U], CervenyImageKind::Surface,
      -1.15263179390808546e3, 1.61115676894208377e1,
      true, 8.09278466911276806e-1,
  ...
  checkComplexNear(
      context, branch.rawImageSum,
      {-2.80901061782714734e-1, -9.59649167089315580e-1},
      1.0e-11, "Munk image sum preserves true/surface order");
  checkComplexNear(
      context, branch.finalContribution,
      {4.84915532011030948e1, -1.36951960913375501e1},
      1.2e-8, "Munk branch final ray contribution");
  checkFloatBits(context, branch.finalContribution, 0x4241f75aU,
                 0xc15b1f86U, "Munk branch legacy quantization");
}

void testRigidReflectionOracle(Context& context) {
  constexpr double launchAngle =
      -7.71226401338552248e-1;
  ...
  context.check(path.points.size() == 798U &&
                    path.steps.size() == 747U &&
                    path.events.size() == 50U,
                "near-boundary rigid ray preserves the Fortran point sequence");
  context.check(
      std::bit_cast<std::uint64_t>(
          path.points.front().slowness.depth) ==
  ...
  const auto epsilon =
      pickMinimumWidthEpsilon(250.0, 1500.0, 1000.0, 2.0);
  ...
  context.checkNear(
      ...
      9.45351890934747030e-1, 1.0e-14,
      "rigid reflection interpolation weight");
  context.checkNear(
      value.interpolatedPosition.depth,
      2.74997008542718504e1, 1.0e-12,
      "rigid reflection interpolated depth");
  checkComplexNear(
      context, value.qInterpolated,
      {5.76209913870936353e6, 3.00000000000000047e6},
      1.0e-8, "rigid reflection interpolated q");
  checkComplexNear(
      context, value.gammaInterpolated,
      {3.51015691548127185e-8, -1.82754171845220517e-8},
      1.0e-20, "rigid reflection interpolated gamma");
  context.check(value.kmahLeft == 1 && value.kmahFinal == 1,
                "rigid reflection KMAH remains positive");
  checkComplexNear(
      context, value.constantCorrected,
      {2.16480708940017799e1, -5.29794011577855617},
      2.0e-14, "rigid reflection corrected constant");
  context.checkNear(
      value.rightPhase, 4.39822971502571107e1, 0.0,
      "rigid reflection accumulates 14 pi phase");
  checkImage(
      context, value.images[0U], CervenyImageKind::True,
      6.85002991457281496e1, 1.34701401038128110e-1,
      true, 1.0,
      {9.84219396825598003e-2, -8.68417319570494395e-1},
      "rigid reflection true image");
  checkImage(
      context, value.images[1U], CervenyImageKind::Surface,
      -1.23499700854271850e2, 4.37843581837594031e-1,
      true, 1.0,
      {-6.40224503426137126e-1, 8.17817268776530049e-2},
      "rigid reflection surface image");
  checkImage(
      context, value.images[2U], CervenyImageKind::Bottom,
      7.65002991457281496e1, 1.68001609932696860e-1,
      true, 1.0,
...
RayState makeSyntheticState(double range) {
  return RayState{
      .position = {.range = range, .depth = 50.0},
...
void testTerminalInactivePointIsRetained(Context& context) {
  ...
  static_cast<void>(influence.accumulate(
      suppressed, path, state, {0.0, 100.0}));
...
void testBranchCutAndHermitePrimitives(Context& context) {
  context.check(
      updateCervenyKmah(
          {-2.4288939311484493e7, -6.9996709949595225e5},
          {-2.2991860400679678e7, 4.2938731999627344e3},
          1) == -1,
      "Munk q imaginary crossing flips KMAH");
  context.check(
      updateCervenyKmah(
          {6.7944202263062631e4, -3.7722193856823377e7},
  ...
  context.expectThrows<ValidationError>(
      [] {
        static_cast<void>(
            updateCervenyKmah({1.0, -1.0}, {-1.0, 1.0}, 2));
      },
      "KMAH contract accepts only the two legacy signs");
  context.checkNear(cervenyHermiteTaper(1.0, 1.0, 2.0),
                    1.0, 0.0, "Hermite inner boundary is inclusive");
  context.checkNear(cervenyHermiteTaper(2.0, 1.0, 2.0),
                    0.0, 0.0, "Hermite outer boundary is inclusive");
  context.checkNear(cervenyHermiteTaper(1.5, 1.0, 2.0),
                    0.5, 0.0, "Hermite cubic midpoint");
}
  ...
  context.expectThrows<ValidationError>(
      [&] {
        ...
        FrequencyWorkspace workspace(
            50.0, ReceiverGrid({40.0, 50.0}, {0.0, 10.0}));
        static_cast<void>(influence.accumulate(
            workspace, path, state, {0.0, 100.0}));
      },
...
int main() {
  ...
  std::cout
      << "All Bellhop F2CPP Cartesian Cerveny influence tests passed\n";
  return 0;
}
