...
using bellhop::SimpleGaussianDiagnostic;
using bellhop::SimpleGaussianDiagnosticRequest;
using bellhop::SimpleGaussianInfluence;
...
void testOriginFormulaAndLegacyStepLength(Context& context) {
  ...
  const std::complex<double> tau{0.05, 0.001};
  const std::complex<double> delay = tau + 0.003 * deltaDepth;
  ...
  context.check(
      std::abs(diagnostic.legacyArcLength -
               weight * segmentLength) > 30.0,
      "legacy SINT fixture differs from true geometric arc length");
  context.checkNear(diagnostic.closestPointDistance,
                    closestPointDistance, 1.0e-15,
                    "simple Gaussian CPA");
...
void testSafeSubsetContracts(Context& context) {
  ...
  context.expectThrows<ValidationError>(
      [&] {
        static_cast<void>(SimpleGaussianInfluence(
            receivers, 10.0, CervenyCoordinateSystem::RayCentered));
      },
      "simple Gaussian rejects ray-centered coordinates");
  context.expectThrows<ValidationError>(
      [&] {
        static_cast<void>(SimpleGaussianInfluence(
            receivers, 10.0, CervenyCoordinateSystem::Cartesian,
            SourceGeometry::Line));
      },
  ...
  context.expectThrows<ValidationError>(
      ...
      "simple Gaussian rejects intensity run modes");
  context.expectThrows<ValidationError>(
      [&] {
        static_cast<void>(SimpleGaussianInfluence(
            ReceiverGrid({15.0, 25.0}, {50.0, 75.0},
                         ReceiverGridLayout::Irregular),
            10.0));
      },
      "simple Gaussian rejects irregular receivers");
  context.expectThrows<ValidationError>(
      [&] {
