...
using bellhop::PchipSsp;
using bellhop::PchipFrequencySsp;
...
SoundSpeedProfile makeMunkPchipProfile() {
  const auto environment = bellhop::test::makeMunkEnvironment();
...
void testTwoPointProfile(Context& context) {
  const PchipSsp profile(SoundSpeedProfile(
      ...
      SspInterpolationKind::Pchip));
  const auto sample = profile.evaluate(
      Vec2{.range = 30.0, .depth = 25.0}, 0U);
...
void testMunkFortranOracle(Context& context) {
  ...
  checkSample(context, profile, 0.0, 0U,
              1.54851999999999998e3, -1.06470000000001619e-1,
              1.51778687423298930e-4, "Munk surface node");
  checkSample(context, profile, 100.0, 0U,
              1.53863544671855811e3, -9.11855328144180050e-2,
              1.53910656288373468e-4, "Munk first-segment midpoint");
  checkSample(context, profile, 225.0, 1U,
              1.52844523412126159e3, -7.19467007783713003e-2,
              1.43250811963000049e-4, "Munk short-segment midpoint");
  checkSample(context, profile, 1500.0, 8U,
              1.50041253509381886e3, 5.17535093819098020e-3,
              3.14929812361940256e-5, "Munk minimum-adjacent midpoint");
}
...
void testPeakPlateauAndExtrapolation(Context& context) {
  const PchipSsp peak(SoundSpeedProfile(
      ...
      SspInterpolationKind::Pchip));
  const auto midpoint = peak.evaluate(
  ...
  context.checkNear(midpoint.soundSpeedHessian.depthDepth, -0.002,
                    1.0e-17, "three-point PCHIP midpoint curvature");
  const auto peakNode = peak.evaluate(
      Vec2{.range = 0.0, .depth = 100.0}, 1U);
  context.checkNear(peakNode.soundSpeedGradient.depth, 0.0, 0.0,
                    "PCHIP forces a zero derivative at a peak");
  const auto extrapolated = peak.evaluate(
  ...
  const PchipSsp plateau(SoundSpeedProfile(
      ...
      SspInterpolationKind::Pchip));
  context.checkNear(
      plateau.evaluate(Vec2{.range = 0.0, .depth = 100.0}, 0U)
          .soundSpeedGradient.depth,
      0.0, 1.0e-15, "PCHIP plateau entry derivative is zero");
  context.checkNear(
      plateau.evaluate(Vec2{.range = 0.0, .depth = 200.0}, 2U)
          .soundSpeedGradient.depth,
...
void testNodeSideSemantics(Context& context) {
  ...
  context.checkNear(right.soundSpeedHessian.depthDepth,
                    7.20140375276255709e-5, 2.0e-18,
                    "right arrival side retains right curvature");
  ...
  context.check(
      profile.evaluate(Vec2{.range = 0.0, .depth = 0.0}, 8U)
              .segmentIndex == 0U,
...
void testFrequencyComplexOracle(Context& context) {
  ...
  const auto firstQuarter = profile.evaluateAtSegment(
  ...
  context.checkNear(firstQuarter.imaginarySoundSpeed,
                    8.64634968092250666, 2.0e-14,
                    "complex PCHIP matches converted-node Fortran oracle");
  const auto secondQuarter = profile.evaluateAtSegment(
      Vec2{.range = 0.0, .depth = 750.0}, 1U);
  context.checkNear(secondQuarter.imaginarySoundSpeed,
                    9.70452545644202935, 2.0e-14,
...
int main() {
  ...
  std::cout << "All Bellhop F2CPP PCHIP SSP tests passed\n";
  return 0;
}
