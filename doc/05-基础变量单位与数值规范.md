...
> 版本：v0.4（2026-07-22）
>
> 适用阶段：二维单频基础版本 → 宽带 Ray-Reuse
>
> 原则：本文是代码、测试和日志共同遵守的数据契约；未写入本文的约定不得成为稳定公共接口。

## 1. 需要项目负责人确认

### 1.1 开工前建议确认：原 Bellhop 与推荐设置对照

原版已有明确值或行为时，表中先列出原值；Ray-Reuse 新实现是否继承或改变，由“推荐设置”说明。你只需要回复类似 `D-01 同意，D-02 选 B……`。

| 编号 | 决策 | 原 Bellhop 值/行为 | Ray-Reuse 推荐设置 | 影响 |
|---|---|---|---|---|
...
| 方案 | 行为 |
|---|---|
| **A（推荐）** | 忽略输入射线数，始终按 `f_max` 和 Bellhop 经验判据生成 `Nalpha_final` |
| B（原版行为） | `Nalpha=0` 时自动计算；`Nalpha>0` 时尊重用户输入，仅在不足时警告 |
| C | `Nalpha=0` 时自动计算；显式输入不足时直接报错，不继续计算 |

当前主设计文档采用方案 A。如果选择 B 或 C，需要同步修改 `01` 和 `02` 文档。

### 1.2 可以等 Fortran oracle 后确认

这些项目不阻塞基础代码开工。原版没有正式阈值时，明确写为“无规定”，而不是猜测一个原值。

| 编号 | 待确认内容 | 原 Bellhop 值/行为 | Ray-Reuse 推荐设置 |
|---|---|---|---|
| **D-07** | 逐步状态比较容差 | 无正式回归容差 | 先用第 9 节暂定值，拿到 oracle 后收紧 |
| **D-08** | 复压力、相位和 TL 容差 | 无正式端到端误差阈值 | 区分内部双精度和 SHD 单精度后再冻结 |
| **D-09** | `dynamicP/dynamicQ` 量纲与表示 | `REAL(KIND=8) p(2),q(2)`；初值 `[1,0]`、`[0,1]`；属于两组基本解 | 保留初始化、归一化和逐分量语义，暂不强加统一量纲 |
| **D-10** | 反射点布局 | `Reflect2D` 保留反射前边界点，并新增一个同位置的反射后点 | 基础对照阶段保留双点布局；事件指向反射前边界点，回归后再评估压缩 |
| **D-11** | SHD 布局 | Fortran direct-access records；压力按默认单精度复数写入 | 首版严格兼容现有 MATLAB reader；内部场保持双精度；HDF5 仅作后续内部主格式候选 |
| **D-12** | 并行确定性 | 原二维主循环串行，按发射角固定顺序累加 | 先保持串行顺序；并行阶段采用有界频率切片独占，每频按固定轨迹顺序累加 |

### 1.3 Ray-Reuse 新增设定：原版无对应值

以下内容在原 Bellhop 中不存在。它们不是“原值迁移”，而是为轨迹复用新增的推荐设计；若无异议，开发时按推荐值执行。
...
| **R-06** | 宽带实施顺序 | 先做逐频完整追踪的非复用基线，再做复用 | 分离宽带 I/O 错误与复用算法错误 |
...
| `travelTime` | 几何走时 | s | `REAL(ray%tau)` |
...
struct SoundSpeedSample {
    ...
    SymmetricMatrix2 soundSpeedHessian;
    double density;
};
```
...
`h/hw0/hw1` 只能作为 `ray_stepper.cpp` 局部变量，不进入公共接口。
...
### 8.1 动态射线 `dynamicP/dynamicQ`（兼容、D-09 待确认）
...
| `travelTime` | `1e-12 s` | `1e-10` |
| `dynamicP/dynamicQ` | `1e-12` | `1e-9` |
| 边界交点 | `1e-8 m` | `1e-10` |

若需放宽，必须记录原因、最大误差和所在射线点。端到端压力/TL 容差等待 D-08。
...
| 边界密度 | g/cm³ | ×1000 → kg/m³（待 D-01） |
| 频率 | Hz | 不变 |
| SHD 压力 | 单精度复数 | writer 显式量化（待 D-03） |
...
| 射线 | `norm(soundSpeed*slowness)≈1`、走时不减、步长为正 |
...
| C-linear SSP | 插值节点、层索引、界面左右极限命名 |
...
| Ray-Reuse | 完整 `RayPathCache`、求积缓存、active mask、反射逐频投影、内存估算 |
