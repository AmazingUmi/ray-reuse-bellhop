...
| **D-11** | SHD 布局 | Fortran direct-access records；压力按默认单精度复数写入 | 首版严格兼容 `test/PlotRead/bellhop_io_py/` reader；内部场保持双精度；HDF5 仅作后续内部主格式候选 |
...
- `steps` 与相邻 `points` 的对应关系必须由单元测试冻结，禁止依赖未记录的循环局部变量；
- `ReflectionEvent::rayPointIndex` 必须指向 `points` 中明确约定的反射前或反射后状态；
